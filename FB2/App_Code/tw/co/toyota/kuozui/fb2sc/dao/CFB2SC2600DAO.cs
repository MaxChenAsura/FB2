...
public class CFB2SC2600DAO : BaseDAO
{
    ...
    public string Month_Close_dao3(string pvSalary_type, string pvSalary_dt, string pvPay_kind, string pvProcess_status, string pSalary_ym)
    {
        try
        {
            ...
            //PRE 7.0  找出員工欠薪還款暫存檔有 但薪資明細檔沒有的資料
            sb.Append(@" select distinct a.EMP_ID from TB_S_M_STAFF_REPAY_TMP a
                                        left join TB_S_S_SALARY_PAY b on a.SALARY_DT = b.SALARY_DT 
                                        and a.EMP_ID = b.EMP_ID 
                                        and a.SALARY_TYPE = b.SALARY_TYPE 
                                        and  a.SALARY_ID = b.PAY_KIND
                                        and b.SALARY_ID=1040
                                        where                                         
                                        b.EMP_ID IS NULL ;");            
            ...
            {
                //將找到的EMP_ID做刪除                 
                sb.Append(" delete from TB_S_M_STAFF_REPAY_TMP");
                sb.Append(" where EMP_ID=@EMP_ID and SALARY_DT=@SALARY_DT and SALARY_TYPE=@SALARY_TYPE and SALARY_ID=@SALARY_ID ;");
                
                ht.Add("@EMP_ID", dt.Rows[i]["EMP_ID"].ToString());
                ht.Add("@SALARY_DT", pvSalary_dt);
                ht.Add("@SALARY_TYPE", pvSalary_type);
                ht.Add("@SALARY_ID", pvPay_kind);
    ...
    public void updtae_VOUCHER_TEMP()
    {
        try
        {
            ...
            //20211013 第二家派遣公司 進項稅額錯誤 故從最後結果改
            sb.Append(@"  update TB_S_VOUCHER_TEMP
                                                 set VochAmt = (select Vochtaxamt from TB_S_VOUCHER_TEMP
				                                                        where Rpamtpes =@Rpamtpes and ISNULL(Vchid,'') <> '')
                                                 where Acct = @Acct and Rpamtpes =@Rpamtpes
                    ");
            ...
            ht.Clear();
/*
            sb.Append(@"  select count(*) as CNT from TB_S_VOUCHER_TEMP
				                          where Rpamtpes =@Rpamtpes and ISNULL(Vchid,'') <> ''
            ");
