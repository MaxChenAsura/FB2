...
/// CFB2SH3400BO 的摘要描述
/// </summary>
public class CFB2SH3400BO : BaseService
{
    ...
    //一括異常註記-(Dtl)
    public string mark(List<Tuple<string, string>> keysListMark, List<Tuple<string, string>> keysList, CFB2SH3400DAO sh040DAO)
    {
        ...
        try
        {
            ...
            if (rtnmessage == "")
            {
                try
                {
                    ...
                    {
                        sh040DAO = new CFB2SH3400DAO();
                        sh040DAO.AWARD_YEAR = item.Item1;
                        sh040DAO.EMP_ID = item.Item2;
    ...
    //駁回-(Dtl)
    public string reject(CFB2SH3400DAO sh040DAO)
    {
        DataTable dt = new DataTable();
        string rtnmessage = "";//存在檢查後的訊息
        try
    ...
    //核可-(Dtl)
    public string approve(CFB2SH3400DAO sh040DAO)
    {
        DataTable dt = new DataTable();
        string rtnmessage = "";//存在檢查後的訊息
        try
    ...
    //前次核可資料比對
    public IWorkbook createExcelFromTemplateToPre(string excelPath, CFB2SH3400DAO sh040DAO)
    {
    ...
    //原始資料比對
    public IWorkbook createExcelFromTemplateOriginal(string excelPath, CFB2SH3400DAO sh040DAO)
    {
