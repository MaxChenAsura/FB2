...
/// CFB2SP0300DAO 的摘要描述
/// </summary>
public class CFB2SP0600DAO : BaseDAO
{
    ...
    public DataTable getExcelData()
    {
        try
        {
            ...
            sb.Append(@"Select ROW_NUMBER() OVER(ORDER BY b.ws_cd,b.emp_id ) As RowNumber
                        ...
                        ,a.REWARD_PAY,a.REWARD_PAY+a.RETIRE_PAY as totalPay																																																																												
                        from TB_S_M_OLD_PENSION_H a																																																																												
                        left join VW_H_EMP_DATA b on a.emp_id=b.emp_id																																																																												
                        where  CONVERT(VARCHAR(7),RETIRE_DT,111) =@RETIRE_YM		
                        and a.COMPUTER_TYPE = 'B' /* 精算 */																																																																										
    ...
    public string getPara1Data()
    {
        try
        {
            ...
            sb.Append(@"select REMARK from TB_9_M_PARAMETER
                        where SYS_CD = 'SP' and MAIN_CD = 'RETIRE_FREETAX1'	
                      ");
            DataTable dt = dbConn.Query(sb, ht);

            if (dt.Rows.Count>0)
	        {
                st = dt.Rows[0]["REMARK"].ToString();
	        }
    ...
    public string getPara2Data()
    {
        try
        {
            ...
            sb.Append(@"select REMARK from TB_9_M_PARAMETER
                        where SYS_CD = 'SP' and MAIN_CD = 'RETIRE_FREETAX2'	
                      ");
            DataTable dt = dbConn.Query(sb, ht);
