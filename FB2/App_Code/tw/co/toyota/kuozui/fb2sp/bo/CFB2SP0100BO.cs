...
/// CFB2SP0100BO 的摘要描述
/// </summary>
public class CFB2SP0100BO : BaseService
{
    public CFB2SP0100BO()
    {
        //
    ...
    public string execute(CFB2SP0100DAO sp0100DAO)
    {
        try
        {
            string rtnmessage = "";
            ...
            //3.已離職不需 試算
            if (sp0100DAO.COMPUTER_TYPE == "A") {
                dt = sp0100DAO.getLeaveDT();
                if ((int)dt.Rows[0]["resultCount"] > 0)
                {  
            ...
            if (sp0100DAO.COMPUTER_TYPE == "B")
            {
                dt = sp0100DAO.getOLDRETIRE();
                if ((int)dt.Rows[0]["resultCount"] > 0)
                {
            ...
            if (rtnmessage == "")
            {
                try
                {
                    sp0100DAO.execSP_S_RETIRE_COMPUTE();
                    rtnmessage += utilities.getSPLOG("SP_S_RETIRE_COMPUTE");
                    if (rtnmessage != "")
                    {
                        return rtnmessage;
                    }
