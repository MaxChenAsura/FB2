...
/// CFB2SP0600BO 的摘要描述
/// </summary>
public class CFB2SP0600BO : BaseService
{
    public CFB2SP0600BO()
    {
        //
    ...
    public IWorkbook createExcelFromTemplateDefault(string excelPath, CFB2SP0600DAO dao)
    {
        ...
        try
        {
            ...
            if (sheet != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    decimal total_REWARD_PAY = 0;//優退金
                    decimal total_get = 0;//退職總額
                    string st1 = "";
                    string st2 = "";
                    st1 = dao.getPara1Data();
                    st2 = dao.getPara2Data();

                    row = sheet.GetRow(7);
                    cell = row.GetCell(0);
                    cell.SetCellValue(dao.RETIRE_YM + " 退休人員名冊暨課稅明細");

                    row = sheet.GetRow(8);
                    cell = row.GetCell(29);                    
                    cell.SetCellValue(string.Format("(1)免稅額{0}年資*" + st1 + "萬", System.Environment.NewLine)); // 有換行
                    //cell.SetCellValue(string.Format("換行{0}測試", System.Environment.NewLine));

                    cell = row.GetCell(32);
                    cell.SetCellValue(string.Format("(2)免稅額{0}年資*" + st2 + "萬", System.Environment.NewLine)); // 有換行
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        total_REWARD_PAY += Convert.ToDecimal(dt.Rows[i]["REWARD_PAY"].ToString());
                        total_get += Convert.ToDecimal(dt.Rows[i]["totalPay"].ToString());
                        ...
                        cell.CellStyle = stringRightStyle;
                        cell.SetCellValue(Convert.ToInt32(dt.Rows[i]["REWARD_PAY"].ToString()).ToString("N0"));
                        ...
                        cell.CellStyle = stringRightStyle;
                        sheet.AddMergedRegion(new CellRangeAddress(x, x, 21, 24));
                        //退職總額 = 實領退休金 + 優退獎勵金
                        cell = row.CreateCell(25);
                        cell.CellStyle = stringRightStyle;
                        cell.SetCellValue(Convert.ToInt32(dt.Rows[i]["totalPay"].ToString()).ToString("N0"));
                        ...
                        cell.CellStyle = stringRightStyle;
                        sheet.AddMergedRegion(new CellRangeAddress(x, x, 25, 28));
                        ...
                        cell.CellStyle = stringRightStyle;
                        sheet.AddMergedRegion(new CellRangeAddress(x, x, 29, 31));
                        //免稅額 2    
                        ...
                        cell.CellStyle = stringRightStyle;
                        sheet.AddMergedRegion(new CellRangeAddress(x, x, 32, 34));
                        //課稅所得 
                        ...
                        cell.CellStyle = stringRightStyle;
                        sheet.AddMergedRegion(new CellRangeAddress(x, x, 35, 37));
                        //扣繳稅額       
                        ...
                        cell.CellStyle = stringRightStyle;
                        sheet.AddMergedRegion(new CellRangeAddress(x, x, 38, 40));
                    }
                    ...
                    cell = row.GetCell(21);
                    cell.SetCellValue(Convert.ToInt32(total_REWARD_PAY).ToString("N0"));
                    sheet.AddMergedRegion(new CellRangeAddress(x, x, 21, 24));

                    cell = row.GetCell(25);
                    cell.SetCellValue(Convert.ToInt32(total_get).ToString("N0"));
                    sheet.AddMergedRegion(new CellRangeAddress(x, x, 25, 28));
