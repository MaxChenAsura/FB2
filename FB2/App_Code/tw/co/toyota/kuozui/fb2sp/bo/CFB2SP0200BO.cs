...
/// CFB2SP020BO 的摘要描述
/// </summary>
public class CFB2SP0200BO : BaseService
{
    public CFB2SP0200BO()
    {
        //
    ...
    public string updateCLOSE_YN(CFB2SP0200DAO sp020DAO)
    {
        string rtnmessage = "";
        try
        {
    ...
    public IWorkbook createExcelFromTemplateDefault(string excelPath, CFB2SP0200DAO sp020DAO)
    {
            ...
            if (sheet != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        cell.CellStyle = stringRightStyle;  //先
                        cell.SetCellValue(Convert.ToInt32(dt.Rows[i]["SPECIAL_PAY"].ToString()).ToString("N0")); //後
                        //小計金額
                    ...
                    cell.SetCellValue(dt.Rows[0]["OLDRETIRE_YEARS"].ToString()); //後
                    cell = row.CreateCell(4);
                    cell.CellStyle = stringLeftStyle;
                    cell.SetCellValue("年");
                    cell = row.CreateCell(5);
                    cell.CellStyle = stringLeftStyle;
                    cell.SetCellValue("新年資");
                    cell = row.CreateCell(6);
                    cell.CellStyle = stringLeftStyle;
                    sheet.AddMergedRegion(new CellRangeAddress(x, x, 5, 6));
                    cell = row.CreateCell(7);
                    cell.CellStyle = stringRightStyle;
                    //新年資 = 在職年資-舊年資
                    ...
                    cell = row.CreateCell(3);
                    cell.CellStyle = stringRightStyle;
                    cell.SetCellValue(dt.Rows[0]["APR_OLDRETIRE_YEARS"].ToString()); //後
                    cell = row.CreateCell(4);
                    cell.CellStyle = stringLeftStyle;
                    cell.SetCellValue("年");
                    cell = row.CreateCell(5);
                    cell.CellStyle = stringLeftStyle;
                    cell.SetCellValue("新退休計算年資");
                    cell = row.CreateCell(6);
                    cell.CellStyle = stringRightStyle;
                    sheet.AddMergedRegion(new CellRangeAddress(x, x, 5, 6));
                    cell = row.CreateCell(7);
