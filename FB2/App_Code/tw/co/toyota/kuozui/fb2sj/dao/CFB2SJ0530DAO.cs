...
/// WFB2SJ0530 的摘要描述
/// </summary>
public class CFB2SJ0530DAO : BaseDAO
{
    ...
    public DataTable getDeptDataByEmpId()
    {
        try
        {
            ...
            sb.Append(@" Select distinct A.DEPT_LEVEL,A.EMP_ID, B.EMP_NAME,
                               (SELECT Count(*) from TB_S_M_ASSESS_DEP20_UP_SIGN  where ASSESS_YEAR=A.ASSESS_YEAR and ASSESS_TYPE=A.ASSESS_TYPE and EMP_ID=A.EMP_ID and (isnull(SIGN_YN,'N')='N' or SIGN_YN='')) SIGN_COUNT
        ...
        try
        {
                ...
                sb.Append(@"
                        and A.DEPT_NO in(
                            ...
                            from TB_H_R_DEPT_DATA_AD SA left join
	                             ...
	                             TB_S_M_ASSESS_DEPT_LEVEL SC on SUBSTRING(SC.LEVEL_RATE,1,len(SB.LEVEL_RATE))=SB.LEVEL_RATE  AND SB.ASSESS_YEAR=SC.ASSESS_YEAR AND SB.ASSESS_TYPE=SC.ASSESS_TYPE
            ...
            if (dept_no != "")
            {
                sb.Append(@" and A.DEPT_NO  in (
                        SELECT SB.DEPT_NO 
                        FROM TB_S_M_ASSESS_DEPT_LEVEL SA   left join 
	                             TB_S_M_ASSESS_DEPT_LEVEL SB on SUBSTRING(SB.LEVEL_RATE,1,len(SA.LEVEL_RATE))=SA.LEVEL_RATE  AND SA.ASSESS_YEAR=SB.ASSESS_YEAR  AND SA.ASSESS_TYPE=SB.ASSESS_TYPE
    ...
    public int getCount(int startRowIndex, int maximumRows, string assess_year, string assess_type,  string ma_emp_id, string level_cd, string score_final,string emp_id,string dept_no,string is_supper)
    {
        try
        {
            ...
            sb.Append("                  TB_S_M_ASSESS_TARGET A  on E.ASSESS_YEAR=A.ASSESS_YEAR and E.ASSESS_TYPE=A.ASSESS_TYPE and E.EMP_ID=A.EMP_ID  ");
