...
/// WFB2SJ0220 的摘要描述
/// </summary>
public class CFB2SJ0220DAO : BaseDAO
{
    ...
    public DataTable getData(int startRowIndex, int maximumRows, string sortExpression, string assess_type)
    {
        try
        {
            ...
            sb.Append(@" A.ASSESS_TYPE, A.ASSESS_TYPE+'-'+D.SUB_DESC as ASSESS_TYPE_DESC,  A.WS_CD, A.LEVEL_CD,
                         ...
                         from TB_S_M_ASSESS_RATE A left join ");
    ...
    public int getCount(int startRowIndex, int maximumRows, string assess_type)
    {
        try
        {
            ...
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("select COUNT(*) total_record from TB_S_M_ASSESS_RATE A ");
            sb.Append(" where 1=1 ");
    ...
    public DataTable getExistData()
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("select ASSESS_TYPE, WS_CD, LEVEL_CD from TB_S_M_ASSESS_RATE ");
            sb.Append(" where ASSESS_TYPE = @ASSESS_TYPE and WS_CD = @WS_CD and LEVEL_CD = @LEVEL_CD ");
            ht.Add("@ASSESS_TYPE", ASSESS_TYPE);
            ht.Add("@WS_CD", WS_CD);
    ...
    public DataTable getUpdData()
    {
        try
        {
            ...
            ");
            sb.Append(@" from TB_S_M_ASSESS_RATE A    left join 
    ...
    public void addRATE()
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("insert into TB_S_M_ASSESS_RATE( ");
            sb.Append(" ASSESS_TYPE , WS_CD , LEVEL_CD , RATE_A , RATE_B , RATE_C , RATE_D , RATE_E , RATE_F , RATE_G ,IS_CTL ");
    ...
    public void updateRATE()
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("update TB_S_M_ASSESS_RATE ");
    ...
    public void deleteRATE(string assess_type, string ws_cd, string level_cd)
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            //寫log
            sb.AppendLine(" update TB_S_M_ASSESS_RATE set UPDATED_DT =getdate(),UPDATED_BY = @CURRENT_EMP,FUNC_ID = 'FB2SJ0220' ");
            sb.Append(" where ASSESS_TYPE = @ASSESS_TYPE and WS_CD = @WS_CD and LEVEL_CD = @LEVEL_CD ;");
            ht.Add("@CURRENT_EMP", SessionHandle.Current.emp_id);

            sb.Append(" delete from TB_S_M_ASSESS_RATE ");
            sb.Append(" where ASSESS_TYPE = @ASSESS_TYPE and WS_CD = @WS_CD and LEVEL_CD = @LEVEL_CD ;");
            ht.Add("@ASSESS_TYPE", assess_type);
            ht.Add("@WS_CD", ws_cd);
            ht.Add("@LEVEL_CD", level_cd);
