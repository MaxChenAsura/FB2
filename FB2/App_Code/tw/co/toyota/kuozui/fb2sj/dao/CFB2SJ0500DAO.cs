...
/// WFB2SJ0500 的摘要描述
/// </summary>
public class CFB2SJ0500DAO : BaseDAO
{
    ...
    public DataTable getData(int startRowIndex, int maximumRows, string sortExpression, string assess_year, string assess_type)
    {
        try
        {
            ...
            sb.Append(@" A.ASSESS_YEAR,A.ASSESS_TYPE, A.DIREC_EMP_ID, B.EMP_NAME, A.DEPT_NO, A.DEPT_FULL_NAME as DEPT_NAME, A.SIGN_YN ,(case when A.SIGN_YN='N' then '未提' else '已提' end) as SIGN_YN_DESC, A.MNG_NUM ,
                        ...
                        from TB_S_M_ASSESS_DIRECTOR_H A INNER JOIN TB_H_M_EMP B on A.DIREC_EMP_ID=B.EMP_ID    
    ...
    public int getCount(int startRowIndex, int maximumRows, string assess_year, string assess_type)
    {
        try
        {
            ...
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("select COUNT(*) total_record from TB_S_M_ASSESS_DIRECTOR_H A ");
            sb.Append(" where 1=1 ");
    ...
    public DataTable getDataDtl(int startRowIndex, int maximumRows, string sortExpression, string assess_year, string assess_type, string dept_no, string dirc_emp_id, string emp_id, string score_dept, string is_out, string level_cd, string ws_cd, string is_dr)
    {
        try
        {
            ...
            sb.Append(@" A.ASSESS_YEAR, A.ASSESS_TYPE, A.EMP_ID, B.EMP_NAME, isnull(B.IS_OUT,'N') IS_OUT,
                        ...
                        ISNULL((SELECT top 1 GRADE FROM TB_S_M_ASSESS_LOG WHERE ASSESS_YEAR=B.ASSESS_YEAR and ASSESS_TYPE=B.ASSESS_TYPE and EMP_ID=B.EMP_ID AND CREATED_BY=D.DIREC_EMP_ID order by created_DT),'') ORI_SCORE_DEPT,	                    
                        SCORE_DEPT,
                        SCORE_DIRC,
                        ...
                        FROM TB_S_M_ASSESS_DIRECTOR_D A left join
                             ...
                             TB_S_M_ASSESS_DIRECTOR_H D on A.ASSESS_YEAR=D.ASSESS_YEAR and A.ASSESS_TYPE=D.ASSESS_TYPE and A.DEPT_NO=D.DEPT_NO AND A.DIREC_EMP_ID=D.DIREC_EMP_ID
    ...
    public DataTable getEmpTargetData()
    {
        try
        {
            ...
            sb.Append(@" Select 
	                    ...
	                    isnull((select sum(MNG_GRADE) from TB_S_M_ASSESS_SCORE WHERE ASSESS_YEAR=A.ASSESS_YEAR and ASSESS_TYPE=A.ASSESS_TYPE and EMP_ID=A.EMP_ID),0) MNG_GRADE,
                        ISNULL((SELECT top 1 GRADE FROM TB_S_M_ASSESS_LOG WHERE ASSESS_YEAR=A.ASSESS_YEAR and ASSESS_TYPE=A.ASSESS_TYPE and EMP_ID=A.EMP_ID AND CREATED_BY=D.DIREC_EMP_ID order by created_DT),'') ORI_SCORE_DEPT,
                        ...
                        FROM TB_S_M_ASSESS_DIRECTOR_D A left join
							 ...
							 TB_9_M_COMM_D F  with (nolock)  on F.SYS_CD='SJ' and F.MAIN_CD='ASSESS_TYPE' and F.SUB_CD= A.ASSESS_TYPE and F.IS_VALID='Y' left join
    ...
    public void addAssessLog_Batch()
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("insert into TB_S_M_ASSESS_LOG ( ");
    ...
    public DataTable getAssessDircH(string assess_year,string assess_type, string dept_no, string direc_emp_id)
    {
        ...
        try
        {
            ...
            sb.Append("select  A.* ");
            sb.Append("FROM TB_S_M_ASSESS_DIRECTOR_H A  ");
            sb.Append("where ASSESS_YEAR=@ASSESS_YEAR and ASSESS_TYPE=@ASSESS_TYPE ");
