...
/// WFB2SJ0130 的摘要描述
/// </summary>
public class CFB2SJ0130DAO : BaseDAO
{
    ...
    public int getCount(int startRowIndex, int maximumRows, string assess_year, string assess_type, string emp_id)
    {
        try
        {
            ...
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("select COUNT(*) total_record from TB_S_M_ASSESS_DISTING_EMP A ");
            sb.Append(" where 1=1 ");
    ...
    public int isExit(string assess_year, string assess_type, string emp_id,String disting_cd,String datasource)
    {
        try
        {
            ...
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("select COUNT(*) total_record from TB_S_M_ASSESS_DISTING_EMP ");
            sb.Append(" where 1=1 ");
    ...
    public void deleteData()
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append(@" delete from TB_S_M_ASSESS_DISTING_EMP 
    ...
    internal void insertData(DateTime now)
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("INSERT INTO TB_S_M_ASSESS_DISTING_EMP (ASSESS_YEAR , ASSESS_TYPE , EMP_ID , DISTING_CD, ");
            sb.Append(" DATASOURCE , REMARK , ABS_SCORE , CHG_WS_CD , EXCEPT_E,");
    ...
    public void updateData()
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("update TB_S_M_ASSESS_DISTING_EMP ");
            sb.Append(" set DATASOURCE = @DATASOURCE,  ");
            sb.Append("     REMARK = @REMARK ,ABS_SCORE = @ABS_SCORE ,CHG_WS_CD = @CHG_WS_CD ,EXCEPT_E = @EXCEPT_E ,UPDATED_BY = @UPDATED_BY,UPDATED_DT = GETDATE()");
        ...
        string cell4, string cell5, string cell6, string cell7)
    {
        try
        {
            ...
            if (cell6.Trim() == "")
                ht.Add("@CHG_WS_CD", DBNull.Value);
            else
                ht.Add("@CHG_WS_CD", cell6.Trim());
            if (cell7.Trim() == "")
                ht.Add("@EXCEPT_E", DBNull.Value);
            else
                ht.Add("@EXCEPT_E", cell7.Trim());
            
            ht.Add("@CREATED_BY", SessionHandle.Current.emp_id);
            ht.Add("@UPDATED_BY", SessionHandle.Current.emp_id);
            ht.Add("@FUNC_ID", "FB2SJ0130");
