...
/// WFB2SJ3700 的摘要描述
/// </summary>
public class CFB2SJ3700DAO : BaseDAO
{
    ...
    public DataTable getDeptDataByEmpId()
    {
        try
        {
            ...
            sb.Append(@" Select 
                         ...
                         from (select * from TB_S_M_FOREIGN_DEPT_LEVEL WHERE  ASSESS_YEAR=@ASSESS_YEAR and ASSESS_TYPE=@ASSESS_TYPE) A left join
                                TB_S_M_FOREIGN_DEP20_UP_SIGN B on A.DEPT_NO=B.DEPT_NO and A.HEAD_EMP_ID=B.EMP_ID and 
    ...
    public DataTable getData(int startRowIndex, int maximumRows, string sortExpression, string assess_year, string assess_type, string dept_no, string ws_cd, string score_level_group, string score_final, string emp_id, string recomm_desc, string dept_emp_id)
    {
        try
        {
            ...
            sb.Append(@" A.ASSESS_YEAR, A.ASSESS_TYPE, A.EMP_ID, A.EMP_NAME, A.DEPT_NO, isnull(A.IS_OUT,'N') IS_OUT,
	                     ...
	                     ((SELECT case when Count(*)>0 then Count(*)-1 else Count(*) end FROM TB_S_M_FOREIGN_LOG WHERE ASSESS_YEAR=A.ASSESS_YEAR and  ASSESS_TYPE=A.ASSESS_TYPE and EMP_ID=A.EMP_ID and 
                            CREATED_BY NOT IN (
                            select DISTINCT MA_EMP_ID from TB_S_M_FOREIGN_DEPT20_MA WHERE ASSESS_YEAR=A.ASSESS_YEAR AND ASSESS_TYPE=A.ASSESS_TYPE
                           ))) FIX_COUNT,
                        ...
                        FROM TB_S_M_FOREIGN_DIRECTOR_D E left join
                             TB_S_M_FOREIGN_TARGET A  on E.ASSESS_YEAR=A.ASSESS_YEAR and E.ASSESS_TYPE=A.ASSESS_TYPE and E.EMP_ID=A.EMP_ID left join
                               ...
                               TB_S_M_FOREIGN_DIRECTOR_H D on E.ASSESS_YEAR=D.ASSESS_YEAR and E.ASSESS_TYPE=D.ASSESS_TYPE and E.DEPT_NO=D.DEPT_NO  AND E.DIREC_EMP_ID=D.DIREC_EMP_ID
            ...
            if (score_level_group != "-1")
            {
                ...
                sb.Append(" )");**/
                sb.Append(" AND A.WS_CD+'-'+A.LEVEL_CD IN (SELECT WS_CD+'-'+LEVEL_CD FROM TB_S_M_FOREIGN_POINT_YEAR WHERE ASSESS_YEAR=E.ASSESS_YEAR AND ASSESS_TYPE=E.ASSESS_TYPE AND POINT_GROUP=@POINT_GROUP) ");
    ...
    public int getCount(int startRowIndex, int maximumRows, string assess_year, string assess_type, string dept_no, string ws_cd, string score_level_group, string score_final, string emp_id, string recomm_desc, string dept_emp_id)
    {
        try
        {
            ...
            sb.Append("                 TB_S_M_FOREIGN_TARGET A  on E.ASSESS_YEAR=A.ASSESS_YEAR and E.ASSESS_TYPE=A.ASSESS_TYPE and E.EMP_ID=A.EMP_ID left join ");
            sb.Append("                  TB_S_M_FOREIGN_DIRECTOR_H D on E.ASSESS_YEAR=D.ASSESS_YEAR and E.ASSESS_TYPE=D.ASSESS_TYPE and E.DEPT_NO=D.DEPT_NO  AND E.DIREC_EMP_ID=D.DIREC_EMP_ID ");
		    ...
		    sb.Append(" FROM ");
            sb.Append("	(select LEVEL_RATE ,ASSESS_YEAR ,ASSESS_TYPE, DEPT_NO, HEAD_EMP_ID from TB_S_M_FOREIGN_DEPT_LEVEL  where  DEPT_LEVEL>='20') X  left join  ");
			sb.Append("					TB_S_M_FOREIGN_DEPT_LEVEL Y on SUBSTRING(Y.LEVEL_RATE,1,len(X.LEVEL_RATE))=X.LEVEL_RATE  and X.ASSESS_YEAR=Y.ASSESS_YEAR and X.ASSESS_TYPE=Y.ASSESS_TYPE ");
        ...
        try
        {
            ...
            if (is_dept_20 == "N")
            {
                ...
                sb.Append(@" A.ASSESS_YEAR,A.ASSESS_TYPE, A.DIREC_EMP_ID, B.EMP_NAME, A.DEPT_NO, A.DEPT_FULL_NAME as DEPT_NAME, A.SIGN_YN ,(case when A.SIGN_YN='N' then '未提' else '已提' end) as SIGN_YN_DESC, A.MNG_NUM ,
                        ...
                        from TB_S_M_FOREIGN_DIRECTOR_H A INNER JOIN 
                             TB_H_M_EMP B on A.DIREC_EMP_ID=B.EMP_ID  JOIN
                             TB_S_M_FOREIGN_DEPT_LEVEL C on A.ASSESS_YEAR=C.ASSESS_YEAR and  A.ASSESS_TYPE=C.ASSESS_TYPE and A.DEPT_NO=C.DEPT_NO and A.LEVEL_RATE=C.LEVEL_RATE
                ...
                sb.Append(@" T.*
                            ...
                            (
                             SELECT  A.ASSESS_YEAR,A.ASSESS_TYPE, A.DIREC_EMP_ID, B.EMP_NAME, A.DEPT_NO, A.DEPT_FULL_NAME as DEPT_NAME, A.SIGN_YN ,(case when A.SIGN_YN='N' then '未提' else '已提' end) as SIGN_YN_DESC, A.MNG_NUM ,
                                     (case when ISNULL(C.SIGN_YN,'N')='Y' OR ISNULL(D.SIGN_YN,'N')='Y' then 'Y' else 'N' end) LEVEL_SIGN_YN ,(case when ISNULL(C.SIGN_YN,'N')='Y' OR ISNULL(D.SIGN_YN,'N')='Y' then '已提' else '未提' end) as LEVEL_SIGN_YN_DESC,
	                                  (Select count(*) from TB_S_M_FOREIGN_TARGET O WHERE O.ASSESS_YEAR=A.ASSESS_YEAR and O.ASSESS_TYPE=A.ASSESS_TYPE and isnull(SCORE_DEPT,'')<>'' and O.EMP_ID IN(
									SELECT P.EMP_ID FROM TB_S_M_FOREIGN_DIRECTOR_D P WHERE P.ASSESS_YEAR=A.ASSESS_YEAR and P.ASSESS_TYPE=A.ASSESS_TYPE and P.DIREC_EMP_ID=A.DIREC_EMP_ID and P.DEPT_NO=A.DEPT_NO) ) as MNG_GRANT_NUM,
	                                (Select count(*) from TB_S_M_FOREIGN_TARGET  O WHERE O.ASSESS_YEAR=A.ASSESS_YEAR and O.ASSESS_TYPE=A.ASSESS_TYPE and isnull(SCORE_DEPT,'')='A' and O.EMP_ID IN(
									SELECT P.EMP_ID FROM TB_S_M_FOREIGN_DIRECTOR_D P WHERE P.ASSESS_YEAR=A.ASSESS_YEAR and P.ASSESS_TYPE=A.ASSESS_TYPE and P.DIREC_EMP_ID=A.DIREC_EMP_ID and P.DEPT_NO=A.DEPT_NO) ) as MNG_GRANT_A_NUM,
	                                (Select count(*) from TB_S_M_FOREIGN_TARGET  O WHERE O.ASSESS_YEAR=A.ASSESS_YEAR and O.ASSESS_TYPE=A.ASSESS_TYPE and isnull(SCORE_DEPT,'')='B' and O.EMP_ID IN(
									SELECT P.EMP_ID FROM TB_S_M_FOREIGN_DIRECTOR_D P WHERE P.ASSESS_YEAR=A.ASSESS_YEAR and P.ASSESS_TYPE=A.ASSESS_TYPE and P.DIREC_EMP_ID=A.DIREC_EMP_ID and P.DEPT_NO=A.DEPT_NO) ) as MNG_GRANT_B_NUM,
	                                (Select count(*) from TB_S_M_FOREIGN_TARGET  O WHERE O.ASSESS_YEAR=A.ASSESS_YEAR and O.ASSESS_TYPE=A.ASSESS_TYPE and isnull(SCORE_DEPT,'')='C' and O.EMP_ID IN(
									SELECT P.EMP_ID FROM TB_S_M_FOREIGN_DIRECTOR_D P WHERE P.ASSESS_YEAR=A.ASSESS_YEAR and P.ASSESS_TYPE=A.ASSESS_TYPE and P.DIREC_EMP_ID=A.DIREC_EMP_ID and P.DEPT_NO=A.DEPT_NO) ) as MNG_GRANT_C_NUM,
	                                (Select count(*) from TB_S_M_FOREIGN_TARGET  O WHERE O.ASSESS_YEAR=A.ASSESS_YEAR and O.ASSESS_TYPE=A.ASSESS_TYPE and isnull(SCORE_DEPT,'')='D' and O.EMP_ID IN(
									SELECT P.EMP_ID FROM TB_S_M_FOREIGN_DIRECTOR_D P WHERE P.ASSESS_YEAR=A.ASSESS_YEAR and P.ASSESS_TYPE=A.ASSESS_TYPE and P.DIREC_EMP_ID=A.DIREC_EMP_ID and P.DEPT_NO=A.DEPT_NO) ) as MNG_GRANT_D_NUM,
	                                (Select count(*) from TB_S_M_FOREIGN_TARGET  O WHERE O.ASSESS_YEAR=A.ASSESS_YEAR and O.ASSESS_TYPE=A.ASSESS_TYPE and isnull(SCORE_DEPT,'')='E' and O.EMP_ID IN(
									SELECT P.EMP_ID FROM TB_S_M_FOREIGN_DIRECTOR_D P WHERE P.ASSESS_YEAR=A.ASSESS_YEAR and P.ASSESS_TYPE=A.ASSESS_TYPE and P.DIREC_EMP_ID=A.DIREC_EMP_ID and P.DEPT_NO=A.DEPT_NO)  ) as MNG_GRANT_E_NUM                         
                              from TB_S_M_FOREIGN_DIRECTOR_H A INNER JOIN 
                                   TB_H_M_EMP B on A.DIREC_EMP_ID=B.EMP_ID  JOIN
                                   TB_S_M_FOREIGN_DEPT_LEVEL C on C.ASSESS_YEAR=A.ASSESS_YEAR and  C.ASSESS_TYPE=A.ASSESS_TYPE and C.DEPT_NO=A.DEPT_NO and C.DEPT_LEVEL<='30' and C.DEPT_LEVEL>10 and C.HEAD_EMP_ID=A.DIREC_EMP_ID LEFT JOIN
                                   TB_S_M_FOREIGN_DEP20_UP_SIGN D ON A.DEPT_NO=D.DEPT_NO and A.DIREC_EMP_ID=D.EMP_ID and 
                            ...
                            UNION
                             SELECT  A.ASSESS_YEAR,A.ASSESS_TYPE, A.DIREC_EMP_ID, B.EMP_NAME, A.DEPT_NO, A.DEPT_FULL_NAME as DEPT_NAME, A.SIGN_YN ,(case when A.SIGN_YN='N' then '未提' else '已提' end) as SIGN_YN_DESC, 
                                      (select sum(MNG_NUM) from TB_S_M_FOREIGN_DIRECTOR_H where ASSESS_YEAR=A.ASSESS_YEAR and ASSESS_TYPE=A.ASSESS_TYPE and DEPT_NO in(
	                                 SELECT Y.DEPT_NO
			                                FROM
				                                (select LEVEL_RATE ,ASSESS_YEAR,ASSESS_TYPE from TB_S_M_FOREIGN_DEPT_LEVEL  where DEPT_NO=A.DEPT_NO and DEPT_LEVEL>='20') X left join 
										                                TB_S_M_FOREIGN_DEPT_LEVEL Y on SUBSTRING(Y.LEVEL_RATE,1,len(X.LEVEL_RATE))=X.LEVEL_RATE  and X.ASSESS_YEAR=Y.ASSESS_YEAR and X.ASSESS_TYPE=Y.ASSESS_TYPE
	                                 ...
	                                 (Select count(*) from TB_S_M_FOREIGN_TARGET O join
                                                           TB_S_M_FOREIGN_DIRECTOR_D P on O.ASSESS_YEAR=P.ASSESS_YEAR and O.ASSESS_TYPE=P.ASSESS_TYPE and O.EMP_ID=P.EMP_ID
                                ...
                                from TB_S_M_FOREIGN_DIRECTOR_H A INNER JOIN 
                                     TB_H_M_EMP B on A.DIREC_EMP_ID=B.EMP_ID  JOIN
                                     TB_S_M_FOREIGN_DEPT_LEVEL C on A.ASSESS_YEAR=C.ASSESS_YEAR and  A.ASSESS_TYPE=C.ASSESS_TYPE and A.DEPT_NO=C.DEPT_NO   
                                where  A.ASSESS_YEAR= @ASSESS_YEAR and A.ASSESS_TYPE= @ASSESS_TYPE and
						            A.DEPT_NO in(
								        SELECT Y.DEPT_NO
								        FROM
    ...
    public int getSituationCount(int startRowIndex, int maximumRows, string assess_year, string assess_type, string dept_no, string is_dept_20, string dept_emp_id)
    {
        try
        {
            ...
            if (is_dept_20 == "N")
            {
                ...
                sb.Append(@" from TB_S_M_FOREIGN_DIRECTOR_H A INNER JOIN TB_H_M_EMP B on A.DIREC_EMP_ID=B.EMP_ID    
                             where  A.ASSESS_YEAR= @ASSESS_YEAR and A.ASSESS_TYPE= @ASSESS_TYPE and 
						            A.LEVEL_RATE in(
								    SELECT Y.LEVEL_RATE
								    FROM                                        
            ...
            else
            {
                ...
                sb.Append(@" select (A.DEPT_NO+'-'+A.DIREC_EMP_ID)DID from TB_S_M_FOREIGN_DIRECTOR_H A INNER JOIN 
                                   TB_H_M_EMP B on A.DIREC_EMP_ID=B.EMP_ID  JOIN
                                   TB_S_M_FOREIGN_DEPT_LEVEL C on C.ASSESS_YEAR=A.ASSESS_YEAR and  C.ASSESS_TYPE=A.ASSESS_TYPE and C.DEPT_NO=A.DEPT_NO and C.DEPT_LEVEL<='30' and C.DEPT_LEVEL>10 and C.HEAD_EMP_ID=A.DIREC_EMP_ID
                              where  A.ASSESS_YEAR= @ASSESS_YEAR and A.ASSESS_TYPE= @ASSESS_TYPE and 
						            A.DEPT_NO in(
								    SELECT Y.DEPT_NO
								    FROM
            ...
            /**
            sb.Append(" select COUNT(*) total_record FROM TB_S_M_FOREIGN_TARGET A ");
            ...
            sb.Append(" FROM ");
            sb.Append("  (select LEVEL_RATE from TB_S_M_FOREIGN_DEPT_LEVEL  where DEPT_NO=@DEPT_NO and DEPT_LEVEL>='20') X left join  ");
            sb.Append("     TB_S_M_FOREIGN_DEPT_LEVEL Y on SUBSTRING(Y.LEVEL_RATE,1,len(X.LEVEL_RATE))=X.LEVEL_RATE  ");
            sb.Append("    )");
            **/
        ...
        try
        {
            ...
            sb.Append(@" SELECT A.ASSESS_YEAR, A.ASSESS_TYPE, A.EMP_ID, A.EMP_NAME, isnull(A.IS_OUT,'N') IS_OUT,
	                           A.LEVEL_CD, A.WS_CD,  A.WS_CD+'-'+B.SUB_DESC as WS_CD_DESC, A.SCORE_DEPT, 
	                            isnull((select sum(MNG_GRADE) from TB_S_M_FOREIGN_SCORE WHERE ASSESS_YEAR=A.ASSESS_YEAR and ASSESS_TYPE=A.ASSESS_TYPE and EMP_ID=A.EMP_ID),0) MNG_GRADE,
	                           (SELECT Count(*) FROM TB_S_M_FOREIGN_LOG WHERE ASSESS_YEAR=A.ASSESS_YEAR and  ASSESS_TYPE=A.ASSESS_TYPE and EMP_ID=A.EMP_ID) FIX_COUNT,
	                            (CASE WHEN A.ASSESS_TYPE='1' then A.SCORE_1H_1 else A.SCORE_2H_1 end) SCORE_H_1,
	                            (CASE WHEN A.ASSESS_TYPE='1' then A.SCORE_1H_2 else A.SCORE_2H_2 end) SCORE_H_2,
		                        A.RECENT_LEVEL_WORK_YEARS, A.AGE, A.WORK_YEARS,isnull(A.DISTING_REMARK,'') DISTING_REMARK,
		                        (A.LEAVE_O+A.LEAVE_P) LEAVE_OP, A.LEAVE_Q, (A.LEAVE_A+A.LEAVE_B) LEAVE_AB
                         FROM TB_S_M_FOREIGN_TARGET A left join
                              TB_S_M_FOREIGN_DIRECTOR_D C ON A.EMP_ID=C.EMP_ID AND A.ASSESS_YEAR=C.ASSESS_YEAR AND A.ASSESS_TYPE=B.ASSESS_TYPE left join
    ...
    public DataTable getEmpAssessRateData()
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append(@" select RATE_A,RATE_B,RATE_C,RATE_D,RATE_E from TB_S_M_FOREIGN_RATE where ASSESS_TYPE=@ASSESS_TYPE and WS_CD=@WS_CD and LEVEL_CD=@LEVEL_CD
                          union
                          select RATE_A,RATE_B,RATE_C,RATE_D,RATE_E from TB_S_M_FOREIGN_RATE where ASSESS_TYPE=@ASSESS_TYPE and WS_CD=@WS_CD  and LEVEL_CD=''");
    ...
    public DataTable getEmpAssessScore(int startRowIndex, int maximumRows, string sortExpression, string assess_year, string assess_type,  string emp_id)
    {
        try
        {
            ...
            Hashtable ht = new Hashtable();
            sb.Append(" select * From");
            sb.Append(" (select ROW_NUMBER() OVER(ORDER BY ITEM_NAME ) As RowNumber,");
            sb.Append(@" ASSESS_YEAR,ASSESS_TYPE, EMP_ID, ITEM_NAME, ITEM_DESC, MAX_GRADE, MNG_GRADE 
    ...
    public DataTable statisticsData()
    {
        ...
        try
        {
            ...
            sb.Append("( ");
            sb.Append("select  P.IS_CTL,'b' RATE_TYPE,'基準' RATE_TYPE_DESC,BASE_A BA, BASE_B BB, BASE_C BC, BASE_D BD, BASE_E BE, BASE_TOT BTOT,");
            sb.Append("       REAL_A RA, REAL_B RB, REAL_C RC, REAL_D RD, REAL_E RE, REAL_TOTAL RTOT,'' CHECK_OK ");
            sb.Append("from TB_S_M_FOREIGN_DEP20_PEO A  left join ");
            sb.Append("       TB_S_M_FOREIGN_RATE P  ON A.ASSESS_TYPE=P.ASSESS_TYPE  ");
            ...
            sb.Append(")X  order by X.RATE_TYPE ");
    ...
    public DataTable statisticsOutData()
    {
        ...
        try
        {
            ...
            sb.Append("select X.* from ");
            sb.Append("( ");
            sb.Append(" select 'o' RATE_TYPE,'外數' RATE_TYPE_DESC,sum(OUT_REAL_A) RA, sum(OUT_REAL_B) RB, sum(OUT_REAL_C) RC, sum(OUT_REAL_D) RD, sum(OUT_REAL_E) RE, sum(OUT_REAL_TOTAL) RTOT ");
            sb.Append("from TB_S_M_FOREIGN_DEP20_PEO A  ");
           sb.Append("where A.ASSESS_YEAR=@ASSESS_YEAR and A.ASSESS_TYPE=@ASSESS_TYPE and A.DEPT_NO_20=@DEPT_NO and A.IS_MERGER<>'A' ");
    ...
    public DataTable referData()
    {
        ...
        try
        {
            ...
            sb.Append("FROM TB_S_M_FOREIGN_TARGET B left join ");
            sb.Append("     TB_9_M_COMM_D C  with (nolock)  on C.SYS_CD='HB' and C.MAIN_CD='WS_CD' and C.SUB_CD= B.WS_CD and C.IS_VALID='Y' left join ");
            sb.Append("     TB_S_M_FOREIGN_DIRECTOR_D A  with (nolock)  on A.ASSESS_YEAR=B.ASSESS_YEAR and A.ASSESS_TYPE=B.ASSESS_TYPE and A.EMP_ID=B.EMP_ID left join   ");
    ...
    public int getNonSignDirectDEPT(String assess_year, String assess_type, String dept_no)
    {
        ...
        try
        {
            ...
            sb.Append(@" select count(*) total_record 
                        ...
                        (
                        select Y.LEVEL_RATE from TB_S_M_FOREIGN_DEPT_LEVEL X LEFT JOIN
                                                              TB_S_M_FOREIGN_DEPT_LEVEL Y ON X.ASSESS_YEAR=Y.ASSESS_YEAR and X.ASSESS_TYPE=Y.ASSESS_TYPE  and SUBSTRING(Y.LEVEL_RATE,1,len(X.LEVEL_RATE))=X.LEVEL_RATE  
        ...
        try
        {
            ...
            for (int i = 0; i < typeArray.Length; i++)
            {
                ...
                for (int j = 0; j < rateArray.Length; j++)
                {
                    ...
                    sb.Append(" from TB_S_M_FOREIGN_DIRECTOR_D B  join   ");
                    sb.Append("  TB_S_M_FOREIGN_TARGET A on A.ASSESS_YEAR=B.ASSESS_YEAR and A.ASSESS_TYPE=B.ASSESS_TYPE and A.EMP_ID=B.EMP_ID  ");
                    ...
                    sb.Append(" 	SELECT Y.LEVEL_RATE  ");
                    sb.Append(" 	FROM  ");
                    sb.Append(" 		(select LEVEL_RATE ,ASSESS_YEAR,ASSESS_TYPE from TB_S_M_FOREIGN_DEPT_LEVEL  where DEPT_NO=@DEPT_NO and DEPT_LEVEL>='20') X left join   ");
                    sb.Append(" 								TB_S_M_FOREIGN_DEPT_LEVEL Y on SUBSTRING(Y.LEVEL_RATE,1,len(X.LEVEL_RATE))=X.LEVEL_RATE  and X.ASSESS_YEAR=Y.ASSESS_YEAR  and X.ASSESS_TYPE=Y.ASSESS_TYPE ");
            ...
            sb.Append(" from TB_S_M_FOREIGN_DEP20_PEO O ");
            sb.Append("  LEFT JOIN TB_S_M_FOREIGN_RATE P  ON O.ASSESS_TYPE=P.ASSESS_TYPE AND O.WS_CD=P.WS_CD AND O.LEVEL_CD=P.LEVEL_CD  ");
        ...
        try
        {
            ...
            sb.Append(@" SELECT distinct A.LEVEL_CD, A.WS_CD,  B.SUB_DESC as WS_CD_DESC
                        FROM TB_S_M_FOREIGN_DIRECTOR_D E left join
                                TB_S_M_FOREIGN_TARGET A  on E.ASSESS_YEAR=A.ASSESS_YEAR and E.ASSESS_TYPE=A.ASSESS_TYPE and E.EMP_ID=A.EMP_ID left join
                                TB_9_M_COMM_D B  with (nolock)  on B.SYS_CD='HB' and B.MAIN_CD='WS_CD' and B.SUB_CD= A.WS_CD and B.IS_VALID='Y'  left join
                                TB_S_M_FOREIGN_DIRECTOR_H D on A.ASSESS_YEAR=D.ASSESS_YEAR and A.ASSESS_TYPE=D.ASSESS_TYPE and A.DEPT_NO=D.DEPT_NO 
    ...
    public DataTable getWSLevelPointData()
    {
        try
        {
            ...
            sb.Append(@" SELECT distinct  A.WS_CD ,A.POINT_GROUP
                        FROM TB_S_M_FOREIGN_POINT_YEAR A 
    ...
    //更新主管簽核TB_S_M_FOREIGN_DEP20_UP_SIGN
    public void updateDEP20_UP_SIGN()
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("update TB_S_M_FOREIGN_DEP20_UP_SIGN ");
    ...
    //更新主管簽核TB_S_M_FOREIGN_DEPT_LEVEL
    public void updateDEPT_SIGN()
    {
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("update TB_S_M_FOREIGN_DEPT_LEVEL ");
            sb.Append(" set SIGN_YN=@SIGN_YN, ");
            sb.Append("     UPDATED_BY = @UPDATED_BY,UPDATED_DT = GETDATE()");
            sb.Append(" where ASSESS_YEAR = @ASSESS_YEAR and ASSESS_TYPE = @ASSESS_TYPE and ");
            sb.Append("       LEVEL_RATE in( ");
            sb.Append(@"Select B.LEVEL_RATE from TB_S_M_FOREIGN_DEPT_LEVEL A LEFT JOIN
    ...
    public DataTable getBackData(int startRowIndex, int maximumRows, string sortExpression, string assess_year, string assess_type, string dept_emp_id)
    {
        try
        {
            ...
            sb.Append(@" X.ASSESS_YEAR, X.ASSESS_TYPE, W.DEPT_NO, W.DEPT_NAME, W.HEAD_EMP_ID, W.HEAD_EMP_NAME, ISNULL(S.SIGN_YN,'N') SIGN_YN
                        FROM
		                    (select LEVEL_RATE ,ASSESS_YEAR ,ASSESS_TYPE, DEPT_NO ,HEAD_EMP_ID from TB_S_M_FOREIGN_DEPT_LEVEL  where  DEPT_LEVEL>='20') X  left join 
	                        TB_S_M_FOREIGN_DEPT_LEVEL Y on SUBSTRING(Y.LEVEL_RATE,1,len(X.LEVEL_RATE))=X.LEVEL_RATE and LEN(Y.LEVEL_RATE)-LEN(X.LEVEL_RATE)<=2 and X.ASSESS_YEAR=Y.ASSESS_YEAR and X.ASSESS_TYPE=Y.ASSESS_TYPE LEFT JOIN
	                        TB_H_R_DEPT_DATA W ON Y.DEPT_NO = W.DEPT_NO LEFT JOIN
                            TB_S_M_FOREIGN_DEPT_LEVEL S ON S.ASSESS_YEAR=X.ASSESS_YEAR and S.ASSESS_TYPE=X.ASSESS_TYPE AND S.DEPT_NO= W.DEPT_NO AND S.DEPT_LEVEL>='20'
                        where X.ASSESS_YEAR=@ASSESS_YEAR and X.ASSESS_TYPE=@ASSESS_TYPE and X.HEAD_EMP_ID=@HEAD_EMP_ID ");
    ...
    //BACK更新主管簽核TB_S_M_FOREIGN_DEPT_LEVEL
    ...
    //BACK更新主管簽核TB_S_M_FOREIGN_Direct_H
    ...
    public DataTable getWSApproveData(int startRowIndex, int maximumRows, string sortExpression, string assess_year, string assess_type, string dept_no, string dept_emp_id)
    {
        try
        {
            ...
            sb.Append(@" SELECT W1.POINT_GROUP SCORE_LEVEL_GROUP,W1.POINT_GROUP ARG,
                            (SELECT distinct SU1.POINT_GROUP_NAME FROM TB_S_M_FOREIGN_POINT_YEAR SU1 WHERE SU1.ASSESS_YEAR=W1.ASSESS_YEAR AND SU1.ASSESS_TYPE=W1.ASSESS_TYPE AND SU1.POINT_GROUP=W1.POINT_GROUP )POINT_GROUP_NAME,
                           (SELECT COUNT(T2.EMP_ID)
		                    FROM   TB_S_M_FOREIGN_DIRECTOR_D T1 LEFT JOIN
			                       TB_S_M_FOREIGN_TARGET T2  ON T1.ASSESS_YEAR = T2.ASSESS_YEAR  AND T1.ASSESS_TYPE = T2.ASSESS_TYPE AND T1.EMP_ID=T2.EMP_ID
		                    WHERE  T1.ASSESS_YEAR=W1.ASSESS_YEAR AND T1.ASSESS_TYPE=W1.ASSESS_TYPE AND T2.EMP_ID<>@HEAD_EMP_ID AND
			                        T2.WS_CD+'-'+T2.LEVEL_CD IN(SELECT WS_CD+'-'+LEVEL_CD FROM TB_S_M_FOREIGN_POINT_YEAR WHERE ASSESS_YEAR=W1.ASSESS_YEAR AND ASSESS_TYPE=@ASSESS_TYPE AND POINT_GROUP=W1.POINT_GROUP) AND
			                       T1.LEVEL_RATE IN(
			                       SELECT Y.LEVEL_RATE
				                    FROM					                    
						                (select LEVEL_RATE ,ASSESS_YEAR ,ASSESS_TYPE, DEPT_NO,HEAD_EMP_ID from TB_S_M_FOREIGN_DEPT_LEVEL  where  DEPT_LEVEL>='20') X   left join 
                    ...
                    FROM TB_S_M_FOREIGN_DEPTPOINT  W1 
    ...
    public DataTable getWSApproveData202410bak(int startRowIndex, int maximumRows, string sortExpression, string assess_year, string assess_type, string dept_no,  string dept_emp_id)
    {
        try
        {
            ...
            sb.Append(@" SELECT W2.SUB_DESC AS WS_CD_DESC,W1.WS_CD,W1.SCORE_LEVEL_GROUP,(W1.WS_CD+'-'+W1.SCORE_LEVEL_GROUP+'-'+ W2.SUB_DESC) ARG,
                    ...
                    FROM TB_S_M_FOREIGN_DEP20_PEO W1 LEFT JOIN
            ...
            sb.Append(@" SELECT distinct W1.POINT_GROUP SCORE_LEVEL_GROUP,
	                            	(W1.POINT_GROUP) ARG,SUM(W1.DEPT_POINT)DEPT_POINT,
                                      ISNULL( (SELECT SUM(T3.POINT)
		                                FROM   TB_S_M_FOREIGN_DIRECTOR_D T1 LEFT JOIN
			                                   TB_S_M_FOREIGN_TARGET T2  ON T1.ASSESS_YEAR = T2.ASSESS_YEAR  AND T1.ASSESS_TYPE = T2.ASSESS_TYPE AND T1.EMP_ID=T2.EMP_ID LEFT JOIN
								               TB_S_M_FOREIGN_GRADEPOINT T3 ON T2.ASSESS_YEAR = T3.ASSESS_YEAR  AND T2.ASSESS_TYPE = T3.ASSESS_TYPE AND T2.SCORE_FINAL=T3.RATING
		                                WHERE  T1.ASSESS_YEAR=W1.ASSESS_YEAR AND T1.ASSESS_TYPE=W1.ASSESS_TYPE AND T2.EMP_ID<>@HEAD_EMP_ID AND
			                                    ISNULL(T2.IS_OUT,'N')='N' AND T2.WS_CD+'-'+T2.LEVEL_CD IN(SELECT WS_CD+'-'+LEVEL_CD FROM TB_S_M_FOREIGN_POINT_YEAR WHERE ASSESS_YEAR=W1.ASSESS_YEAR AND ASSESS_TYPE=@ASSESS_TYPE AND POINT_GROUP=W1.POINT_GROUP) AND
			                                   T1.LEVEL_RATE IN(
			                                   SELECT Y.LEVEL_RATE
				                                FROM					                    
						                            (select LEVEL_RATE ,ASSESS_YEAR ,ASSESS_TYPE, DEPT_NO,HEAD_EMP_ID from TB_S_M_FOREIGN_DEPT_LEVEL  where  DEPT_LEVEL>='20') X   left join 
