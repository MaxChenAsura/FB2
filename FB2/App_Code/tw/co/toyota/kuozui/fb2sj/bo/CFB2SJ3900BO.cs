...
/// CFB2SJ3900BO 的摘要描述
/// </summary>
public class CFB2SJ3900BO : BaseService
{
    ...
    //新增
    public string insertData(CFB2SJ3900DAO sj390DAO)
    {
        string rtnmessage = "";
        try
        {
            ...
            //若需要則要進行邏輯檢查(與DB相關的)
            //00.檢查PK值有無重覆
            DataTable dupdata = sj390DAO.getPKData();
    ...
    {
        CFB2SJ3900DAO sj390DAO = new CFB2SJ3900DAO();
        ...
        try
        {
            ...
            if (rtnmessage == "")
            {
                try
                {
                    ...
                    {
                        //刪除 考核資料維護檔
                        sj390DAO.deleteData_H(item.Item1, item.Item2);
                        //刪除 考核人事資料主檔
                        sj390DAO.deleteData_D(item.Item1, item.Item2, "TB_S_R_ASSESS_TARGET");
                        //刪除 考核人事資料維護檔
                        sj390DAO.deleteData_D(item.Item1, item.Item2, "TB_S_M_ASSESS_TARGET");
                    }
                    Commit();
                    return "0";
                }
    ...
    //考核對象生成
    public string execSP_S_ASSESS_DATA(CFB2SJ3900DAO sj390DAO)
    {
        string rtnmessage = "";//存在檢查後的訊息
        ...
        try
        {
            ...
            //DataTable ckDT = sj0150BO.getNoSetDtlGroupH(sj390DAO.ASSESS_YEAR, sj390DAO.ASSESS_TYPE);
           ...
           // }
            if (rtnmessage == "")
            {
                ...
                sj390DAO.execSP_S_ASSESS_DATA();
                
                rtnmessage += utilities.getSPLOG("SP_S_ASSESS_DATA");
                if (rtnmessage != "") {
                    return rtnmessage;
                }
                sj390DAO.execSP_S_ASSESS_L2_DATA();
                ...
                dt_as400 = sj390DAO.getCMB10EMP();
                //dt = sj390DAO.getAssessEmpData();  //無AS400環境測試時要註解
                BeginTransaction();
                foreach (DataRow dr in dt_as400.Rows)
                {
                    //sj390DAO.EMP_ID = Convert.ToString(dr["EMP_ID"]);
                    //dt_as400 = sj390DAO.getCMB10();
                    sj390DAO.EMP_ID = dr["EMP_ID"].ToString() != "" ? dr["EMP_ID"].ToString() : "0";
                    sj390DAO.PROPOSAL_TOTAL = dr["total"].ToString() != "" ? dr["total"].ToString() : "0";
                    sj390DAO.PROPOSAL_GRADE = dr["grade"].ToString() != "" ? dr["grade"].ToString() : "0";
                    sj390DAO.PROPOSAL_GRADE_MEAN = dr["mean"].ToString() != "" ? dr["mean"].ToString() : "0";
                    sj390DAO.PROPOSAL_6 = dr["LTotal"].ToString() != "" ? dr["LTotal"].ToString() : "0";

                    sj390DAO.updateData(now);

                }
                sj390DAO.updateMeanData(now);
    ...
    //考核一括維護
    public string execSP_S_ASSESS_UPDATE_SCORE(CFB2SJ3900DAO sj390DAO)
    {
        string rtnmessage = "";//存在檢查後的訊息
        ...
        try
        {
            ...
            if (rtnmessage == "")
            {
                sj390DAO.execSP_S_ASSESS_UPDATE_SCORE();
                rtnmessage += utilities.getSPLOG("SP_S_FASSESS_UPDATE_SCORE");
                if (rtnmessage != "")
                {
                    return rtnmessage;
                }
    ...
    //提出核可
    public string updateRelease(CFB2SJ3900DAO sj390DAO)
    {
    ...
    //考核發佈
    public string updateAnnounce(CFB2SJ3900DAO sj390DAO)
    {
        ...
        try
        {
            ...
            //更新考績主檔
            updateAnnounceFlow(sj390DAO);
            

            //發佈完成後更新考核資料維護檔
            DateTime now = DateTime.Parse(DateTime.Now.ToString());
            sj390DAO.updateAnnounce(now);
    ...
    //更新考績主檔
    public void updateAnnounceFlow(CFB2SJ3900DAO sj390DAO)
    {
        //要再加一次新增或者是更新到考績主檔(先insert到考績主檔再進行更新考核資料維護檔,以防無法重按)
        if (sj390DAO.ASSESS_TYPE == "1")
        {
            sj390DAO.delete_M_ASSESS();
        }
        //以年度和類別取得考核發佈資料
        DataTable dtDta = sj390DAO.get_R_ASSESS();
        foreach (DataRow row in dtDta.Rows)
        {
            sj390DAO.EMP_ID = row["EMP_ID"].ToString();
            //類型為"1" 新增
            if (sj390DAO.ASSESS_TYPE == "1")
            {
                //EMP_ID, SCORE_FINAL, DEPT_FLAG, LEVELUP_FLAG, DEPT_NO, DEPT_NAME
                sj390DAO.SCORE_1H = row["SCORE_FINAL"].ToString();
                sj390DAO.DEPT_FLAG_1H = row["DEPT_FLAG"].ToString();
                sj390DAO.LEVEL_FLAG_1H = row["LEVELUP_FLAG"].ToString();
                sj390DAO.DEPT_NO_1H = row["DEPT_NO"].ToString();
                sj390DAO.DEPT_NAME_1H = row["DEPT_NAME"].ToString();

                sj390DAO.insert_M_Assess1();
            }
            //類型為"2" 
            if (sj390DAO.ASSESS_TYPE == "2")
            {
                sj390DAO.SCORE_2H = row["SCORE_FINAL"].ToString();
                sj390DAO.DEPT_FLAG_2H = row["DEPT_FLAG"].ToString();
                sj390DAO.LEVEL_FLAG_2H = row["LEVELUP_FLAG"].ToString();
                sj390DAO.DEPT_NO_2H = row["DEPT_NO"].ToString();
                sj390DAO.DEPT_NAME_2H = row["DEPT_NAME"].ToString();
                //存在則更新，不存在新增
                if (sj390DAO.isAssessExist())
                {
                    sj390DAO.update_M_Assess();
                }
                else
                {
                    sj390DAO.insert_M_Assess2();
                }
            }
        }
    }
    ...
    //考績一括更新(Dtl)
    public string updateAssessScore_ALL(List<Tuple<string, string, string, string>> keysList, string assess_score, CFB2SJ3900DAO sj390DAO)
    {
        ...
        try
        {
            ...
            foreach (var item in keysList)
            {
                ...
                //能力考課時,
                if (sj390DAO.ASSESS_TYPE == "1" && "SFGHIJ".IndexOf(assess_score) > -1)
                {
                    //檢查2S 考績才能  SFGHIJ
                    if (item.Item4 != "2S")
                    {
            ...
            if (rtnmessage == "")
            {
                try
                {
                    ...
                    //00.是否已核可或駁回過
                    DataTable dupdata = sj390DAO.getIsApproveOrReject();
                    ...
                    foreach (var item in keysList)
                    {
                        sj390DAO = new CFB2SJ3900DAO();
                        sj390DAO.ASSESS_YEAR = item.Item1;
                        sj390DAO.ASSESS_TYPE = item.Item2;
                        sj390DAO.EMP_ID = item.Item3;
                        sj390DAO.SCORE_DEPT = assess_score;
                        sj390DAO.SCORE_FINAL = assess_score;
                        sj390DAO.UPDATED_BY = SessionHandle.Current.emp_id;
                        sj390DAO.FUNC_ID = "FB2SJ390";
                        //考核人事資料維護檔
                        sj390DAO.updateAssessScore_ALL(now);
                        //考課等級LOG檔
                        ...
                        sj050DAO.FUNC_ID = "FB2SJ390";
                        sj050DAO.addAssessLog();
                        //若核可狀態為駁回或核可，則異動狀態為V
                        if (isApproveOrReject) {
                            sj390DAO.updateChgStatus_D(now);
                        }


                    }
                    //更新考核資料維護檔為回復成未核可前狀態
                    sj390DAO.updateRejectData_H(now);
                    Commit();
                    return "0";
                }
                catch (Exception ex)
    ...
    //最終考績一括更新(Dtl)
    public string updateAssessScore_Final(List<Tuple<string, string, string, string>> keysList, string assess_score, CFB2SJ3900DAO sj390DAO)
    {
        CFB2SJ0500DAO sj050DAO;
        DataTable dt = new DataTable();
        string rtnmessage = "";//存在檢查後的訊息
            ...
            if (rtnmessage == "")
            {
                try
                {
                    ...
                    foreach (var item in keysList)
                    {
                        ...
                        //考核人事資料維護檔
                        sj390DAO.updateAssessScore_Final(now);
                        //考課等級LOG檔
                        sj050DAO = new CFB2SJ0500DAO();
                        sj050DAO.ASSESS_YEAR = item.Item1;
                        sj050DAO.ASSESS_TYPE = item.Item2;
    ...
    //處理考核表通知部長部門未結通知作業
    public string dept20NotifyMail(CFB2SJ3900DAO sj390DAO)
    {
        try
        {
            sj390DAO.execSP_S_ASSESS_DEP20_MAIL_CHKDT_MAIL();
    ...
    public IWorkbook createExcelResult(string excelPath, CFB2SJ3900DAO sj390DAO)
    {
        ...
        {
            //取得下載資料
            DataTable dt = sj390DAO.getExcelResultData();
            ...
            if (sheet != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    row = sheet.GetRow(0);
                    cell = row.GetCell(0);
                    cell.SetCellValue(sj390DAO.ASSESS_YEAR + "年" + dt.Rows[0]["ASSESS_TYPE_DESC"].ToString() + "結果一覽");
    ...
    public void createPrintExcels_EMPTY(HttpServerUtility Server, string toPath, CFB2SJ3900DAO sj390DAO)
    {
        int totalPage = 0;
        FileStream fs = null;
        IWorkbook workbook = null;
    ...
    #region 考核表列印(新)
    public string createPrintExcels_NEW(HttpServerUtility Server, string toPath, CFB2SJ3900DAO sj390DAO)
    {
        try
        {
            DataTable dt = new DataTable();

            //dt = sj390DAO.getDeptNO("KB00000"); //測試某部門時
            dt = sj390DAO.getDeptNO();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    sj390DAO.DEPT_NO_20 = Convert.ToString(dr["DEPT_NO_20"]);
                    sj390DAO.DEPT_NAME_20 = Convert.ToString(dr["DEPT_NAME_20"]);
                    
                    //title與年度相關的資料
                    sj390DAO.year_title = sj390DAO.ASSESS_YEAR;                                           //今年度 如2014
                    sj390DAO.year_1_title = Convert.ToString(Convert.ToInt32(sj390DAO.ASSESS_YEAR) - 1);  //前1年度 如2014-1
                    sj390DAO.year_2_title = Convert.ToString(Convert.ToInt32(sj390DAO.ASSESS_YEAR) - 2);  //前2年度 如2014-2

                    //部門長官的迴圈
                    for (int i = 0; i < dept_level_arrya.Length; i++)
                    {
                        sj390DAO.dept_level = dept_level_arrya[i];
                        sj390DAO.dept_level_name = dept_name_array[i];

                        if (sj390DAO.ASSESS_TYPE == "1")
                        {
                            createDeptStaffExcel_NEW(Server, toPath, sj390DAO);
                            createDeptWorkerExcel_NEW(Server, toPath, sj390DAO);
                        }
                        else {
                            createDeptStaffExcel_NEW(Server, toPath, sj390DAO);
                            createDeptWorkerExcel_NEW(Server, toPath, sj390DAO);
    ...
    public IWorkbook createExcelFromTemplateDefault(string excelPath, CFB2SJ3900DAO sj390DAO, string functionID)
    {
    ...
    //產生事務系的EXCEL檔
    public string createDeptStaffExcel_NEW(HttpServerUtility Server, string toPath, CFB2SJ3900DAO sj390DAO)
    {
        string assessType = sj390DAO.ASSESS_TYPE;
        int totalPage = 0;
        FileStream fs = null;
        IWorkbook workbook = null;
        ...
        try
        {
            ...
            //計算該部門「事務系」的人數及分頁的頁數
            this.pageIndex = 0;
            totalPage = sj390DAO.getDept_Level_Total("S");
            dt_dept_level = sj390DAO.getDept_Level_Data("S");
            ...
            if (sheet != null)
            {
                ...
                {
                    sj390DAO.HEAD_EMP_ID = dt_dept_level.Rows[j]["EMP_ID"].ToString();
                    sj390DAO.HEAD_EMP_NAME = dt_dept_level.Rows[j]["EMP_NAME"].ToString();
                    sj390DAO.HEAD_DEPT_FULL_NAME = dt_dept_level.Rows[j]["DEPT_FULL_NAME"].ToString();

                    //取得要匯出的員工
                    dt = sj390DAO.getExport_Data("S");

                    if (assessType == "1")
                    {
                        sheet = insertExcelType_NEW(sheet, sj390DAO, dt);
                    }
                    else {
                        sheet = insertExcelType_NEW(sheet, sj390DAO, dt);
                    }
                ...
                string dep_no = sj390DAO.DEPT_NO_20 + "_" + sj390DAO.DEPT_NAME_20 + "_S_" + sj390DAO.dept_level_name;
    ...
    //產生現場系系的Excel檔
    public string createDeptWorkerExcel_NEW(HttpServerUtility Server, string toPath, CFB2SJ3900DAO sj390DAO)
    {
        string assessType = sj390DAO.ASSESS_TYPE;
        int totalPage = 0;
        FileStream fs = null;
        ...
        try
        {
            ...
            //計算該部門「事務系」的人數及分頁的頁數
            this.pageIndex = 0;
            totalPage = sj390DAO.getDept_Level_Total("W");
            dt_dept_level = sj390DAO.getDept_Level_Data("W");
            ...
            if (sheet != null)
            {
                ...
                string dep_no = sj390DAO.DEPT_NO_20 + "_" + sj390DAO.DEPT_NAME_20 + "_W_" + sj390DAO.dept_level_name;
                FileStream file = new FileStream(@toPath + "/" + dep_no + ".xlsx", FileMode.Create);//產生檔案
                workbook.Write(file);
                file.Close();
    ...
    public ISheet insertExcelType_NEW(ISheet sheet, CFB2SJ3900DAO sj390DAO, DataTable dt)
    {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if ((i + 1) % 10 == 1)
                    {
                        ...
                        cell_G.SetCellValue(sj390DAO.year_title);

                        //配佈單位
                        row_G = sheet.GetRow(2 + row_number);
                        cell_G = row_G.GetCell(9);
                        cell_G.SetCellValue(sj390DAO.HEAD_DEPT_FULL_NAME);
                        //初核者姓名
                        row_G = sheet.GetRow(3 + row_number);
                        cell_G = row_G.GetCell(9);
                        cell_G.SetCellValue(sj390DAO.HEAD_EMP_ID + sj390DAO.HEAD_EMP_NAME);
                        ...
                        if (sj390DAO.ASSESS_TYPE == "1")
                        {
                            ...
                            cell_G.SetCellValue(sj390DAO.year_2_title);
                            cell_G = row_G.GetCell(63);
                            cell_G.SetCellValue(sj390DAO.year_1_title);
    ...
    //考核表列印
    public string createPrintExcels(HttpServerUtility Server, string toPath, CFB2SJ3900DAO sj390DAO)
    {
        try
        {
            DataTable dt = new DataTable();
            //因為部門代號觀音及中壢不會有相關的部級部門代號，故不用工廠區分
            //dt = sj390DAO.getDeptNO("KH00000"); //測試某部門時
            dt = sj390DAO.getDeptNO();
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        //產生staff的資料
                        createDeptStaffExcel(Server, toPath, sj390DAO, "1");
                        //產生worker的資料
                        createDeptWorkerExcel(Server, toPath, sj390DAO, "1");
                    }
                    else if (sj390DAO.ASSESS_TYPE == "2")
                    {
                        //產生staff的資料
                        createDeptStaffExcel(Server, toPath, sj390DAO, "2");
                        //產生worker的資料
                        createDeptWorkerExcel(Server, toPath, sj390DAO, "2");
    ...
    public string createDeptWorkerExcel(HttpServerUtility Server, string toPath, CFB2SJ3900DAO sj390DAO, string assessType)
    {
        int totalPage = 0;
        FileStream fs = null;
        IWorkbook workbook = null;
        ...
        try
        {
            ...
            this.pageIndex = 0;
            //Worker
            dt_3A4B_W = sj390DAO.getDeptWorker_3A4B();
            totalPage += getPage(dt_3A4B_W);
            dt_5A_W = sj390DAO.getDeptWorker_5A();
            ...
            if (sheet != null)
            {
                ...
                {
                    sj390DAO.WS_CD = dt_3A4B_W.Rows[j]["WS_CD"].ToString();
                    sj390DAO.LEVEL_CD = dt_3A4B_W.Rows[j]["LEVEL_CD"].ToString();
                    sj390DAO.PLANT_CD = dt_3A4B_W.Rows[j]["PLANT_CD"].ToString();
                    sj390DAO.DEPT_NO_40 = dt_3A4B_W.Rows[j]["DEPT_NO_40"].ToString();
                    sj390DAO.levelCD_title = sj390DAO.LEVEL_CD;  //資格
                    sj390DAO.levelCD_range_title = getTitle_levelCDRange(sj390DAO); //資格區間
                    sj390DAO.plantCD_title = getTitle_Plant(sj390DAO);  //資格(廠別)

                    //員工的迴圈
                    dt = sj390DAO.getWorker_3A4B();
                    sj390DAO.deptName_title = getTitle_deptName(dt, "2");//部門名稱 

                    if (assessType == "1")
                    {
                        sheet = insertWorkerExcelType_1(sheet, sj390DAO, dt, this.pageIndex);
                    }
                    else
                    {
                        sheet = insertWorkerExcelType_2(sheet, sj390DAO, dt, this.pageIndex);
                    }
                    //當為10的倍數時不要加1頁
                    if (dt.Rows.Count % 10 != 0)
                    {
                ...
                {
                    sj390DAO.WS_CD = dt_5A_W.Rows[j]["WS_CD"].ToString();
                    sj390DAO.LEVEL_CD = dt_5A_W.Rows[j]["LEVEL_CD"].ToString();
                    sj390DAO.PLANT_CD = dt_5A_W.Rows[j]["PLANT_CD"].ToString();
                    sj390DAO.DEPT_NO = dt_5A_W.Rows[j]["DEPT_NO"].ToString();
                    sj390DAO.levelCD_title = sj390DAO.LEVEL_CD;  //資格
                    sj390DAO.levelCD_range_title = getTitle_levelCDRange(sj390DAO); //資格區間
                    sj390DAO.plantCD_title = getTitle_Plant(sj390DAO);  //資格(廠別)

                    //員工的迴圈
                    dt = sj390DAO.getWorker_5A();
                    sj390DAO.deptName_title = getTitle_deptName(dt, "6");//部門名稱 
                ...
                string dep_no = sj390DAO.DEPT_NO_20 + "_W";
                FileStream file = new FileStream(@toPath + "/" + dep_no + ".xlsx", FileMode.Create);//產生檔案
                workbook.Write(file);
                file.Close();
    ...
    public ISheet insertWorkerExcelType_1(ISheet sheet, CFB2SJ3900DAO sj390DAO, DataTable dt, int pageIndex)
    {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if ((i + 1) % 10 == 1)
                    {
                        ...
                        cell_G.SetCellValue(sj390DAO.levelCD_range_title);
                        cell_G = row_G.GetCell(78);
                        cell_G.SetCellValue(sj390DAO.plantCD_title);

                        //部門名稱
                        row_G = sheet.GetRow(3 + x);
                        cell_G = row_G.CreateCell(0);
                        cell_G.SetCellValue(sj390DAO.deptName_title);
                        //資格
                        cell_G = row_G.CreateCell(67);
                        cell_G.SetCellValue(sj390DAO.levelCD_title);
    ...
    public ISheet insertWorkerExcelType_2(ISheet sheet, CFB2SJ3900DAO sj390DAO, DataTable dt, int pageIndex)
    {
    ...
    public string createDeptStaffExcel(HttpServerUtility Server, string toPath, CFB2SJ3900DAO sj390DAO, string assessType)
    {
        int totalPage = 0;
        FileStream fs = null;
        IWorkbook workbook = null;
        ...
        try
        {
            ...
            //計算該部門「事務系」的人數及分頁的頁數
            this.pageIndex = 0;
            dt_3A3B = sj390DAO.getDeptStaff_3A3B();
            totalPage += getPage(dt_3A3B);
            dt_4A5A = sj390DAO.getDeptStaff_4A5A();
            totalPage += getPage(dt_4A5A);
            //G-業務
            dt_G = sj390DAO.getDeptWorker_G();
            totalPage += getPage(dt_G);
            //T-特勤
            dt_T = sj390DAO.getDeptWorker_T();
            ...
            if (sheet != null)
            {
                ...
                {
                    sj390DAO.WS_CD = dt_3A3B.Rows[j]["WS_CD"].ToString();
                    sj390DAO.LEVEL_CD = dt_3A3B.Rows[j]["LEVEL_CD"].ToString();
                    sj390DAO.PLANT_CD = dt_3A3B.Rows[j]["PLANT_CD"].ToString();
                    sj390DAO.DEPT_NO_20 = dt_3A3B.Rows[j]["DEPT_NO_20"].ToString();
                    sj390DAO.DEPT_NAME_30 = dt_3A3B.Rows[j]["DEPT_NAME_30"].ToString();
                    ...
                    //員工的迴圈
                    dt = sj390DAO.getStaff_3A3B();
                    sj390DAO.deptName_title = getTitle_deptName(dt, "1");//部門名稱 

                    if (assessType == "1")
                    {
                        sheet = insertStaffExcelType_1(sheet, sj390DAO, dt, this.pageIndex);
                    }
                    else
                    {
                        sheet = insertStaffExcelType_2(sheet, sj390DAO, dt, this.pageIndex);
                    }
                ...
                {
                    sj390DAO.WS_CD = dt_4A5A.Rows[j]["WS_CD"].ToString();
                    //sj390DAO.LEVEL_CD = dt_4A5A.Rows[j]["LEVEL_CD"].ToString();
                    sj390DAO.PLANT_CD = dt_4A5A.Rows[j]["PLANT_CD"].ToString();
                    sj390DAO.DEPT_NO_40 = dt_4A5A.Rows[j]["DEPT_NO_40"].ToString();
                    //title資料
                    //sj390DAO.year_title = sj390DAO.ASSESS_YEAR;  //年度 如2014
                    //sj390DAO.year_1_title = Convert.ToString(Convert.ToInt32(sj390DAO.ASSESS_YEAR) - 1);  //前1年度 如2014
                    //sj390DAO.year_2_title = Convert.ToString(Convert.ToInt32(sj390DAO.ASSESS_YEAR) - 2);  //前2年度 如2014
                    
                    //sj390DAO.levelCD_title = sj390DAO.LEVEL_CD;  //資格
                    //sj390DAO.levelCD_range_title = getTitle_levelCDRange(sj390DAO); //資格區間
                    sj390DAO.levelCD_title = "4A~5A";  //資格
                    sj390DAO.levelCD_range_title = "4A~5A"; //資格區間
                    sj390DAO.plantCD_title = getTitle_Plant(sj390DAO);  //(廠別)

                    //員工的迴圈
                    dt = sj390DAO.getStaff_4A5A();
                    sj390DAO.deptName_title = getTitle_deptName(dt, "2");//部門名稱 
                    if (assessType == "1")
                    {
                        sheet = insertStaffExcelType_1(sheet, sj390DAO, dt, this.pageIndex);
                ...
                {
                    sj390DAO.WS_CD = dt_G.Rows[j]["WS_CD"].ToString();
                    //sj390DAO.LEVEL_CD = dt_G.Rows[j]["LEVEL_CD"].ToString();
                    sj390DAO.PLANT_CD = dt_G.Rows[j]["PLANT_CD"].ToString();
                    sj390DAO.DEPT_NO_40 = dt_G.Rows[j]["DEPT_NO_40"].ToString();
                    sj390DAO.levelCD_range_title = "業務職"; //資格區間
                    sj390DAO.plantCD_title = getTitle_Plant(sj390DAO);  //資格(廠別)


                    //員工的迴圈
                    dt = sj390DAO.getWorker_G();
                    sj390DAO.deptName_title = getTitle_deptName(dt, "2");//部門名稱 
                    sj390DAO.levelCD_title = "RA~RB";
                    if (assessType == "1")
                    {
                        sheet = insertStaffExcelType_1(sheet, sj390DAO, dt, this.pageIndex);
                    }
                ...
                {
                    sj390DAO.WS_CD = dt_T.Rows[j]["WS_CD"].ToString();
                    //sj390DAO.LEVEL_CD = dt_T.Rows[j]["LEVEL_CD"].ToString();
                    sj390DAO.PLANT_CD = dt_T.Rows[j]["PLANT_CD"].ToString();
                    sj390DAO.DEPT_NO_40 = dt_T.Rows[j]["DEPT_NO_40"].ToString();
                    //sj390DAO.PJOB_CD = dt_T.Rows[j]["PJOB_CD"].ToString();
                    sj390DAO.levelCD_title = "4A~5A"; //單一資格
                    sj390DAO.levelCD_range_title = "特勤人員"; //資格區間
                    sj390DAO.plantCD_title = getTitle_Plant(sj390DAO);  //(廠別)

                    //員工的迴圈
                    dt = sj390DAO.getWorker_T();
                    sj390DAO.deptName_title = getTitle_deptName(dt, "2");//部門名稱 
                ...
                string dep_no = sj390DAO.DEPT_NO_20 + "_S";
                FileStream file = new FileStream(@toPath + "/" + dep_no + ".xlsx", FileMode.Create);//產生檔案
                workbook.Write(file);
                file.Close();
    ...
    public ISheet insertStaffExcelType_1(ISheet sheet, CFB2SJ3900DAO sj390DAO, DataTable dt, int pageIndex)
    {
    ...
    public ISheet insertStaffExcelType_2(ISheet sheet, CFB2SJ3900DAO sj390DAO, DataTable dt, int pageIndex)
    {
        try
        {
            //IRow row_G; //改類別變數
    ...
    //每頁的title資料_工廠
    private string getTitle_Plant(CFB2SJ3900DAO sj390DAO)
    {
        if (sj390DAO.PLANT_CD == "1")
        {
            return "(中壢)";
        }
        else
    ...
    //每頁的title資料_資格區間
    private string getTitle_levelCDRange(CFB2SJ3900DAO sj390DAO)
    {
        string result = "";
        if (sj390DAO.LEVEL_CD.IndexOf("3") > -1)
        {
            result = "3A~3B";
        }
        else if (sj390DAO.LEVEL_CD.IndexOf("4") > -1)
        {
            result = "4A~4B";
        }
        else if (sj390DAO.LEVEL_CD.IndexOf("5") > -1)
        {
            result = "5A";
        }
        else if (sj390DAO.LEVEL_CD.IndexOf("R") > -1)
        {
            result = "RA~RB";
        }
        else
