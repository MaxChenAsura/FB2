...
/// CFB2DE0800DAO 的摘要描述
/// </summary>
public class CFB2DE0800DAO : BaseDAO
{
    //Session
    ...
    public DataTable getManagerDT(string MANAGER_YM)
    {
        ...
        try
        {
            ...
            sb.Append(@"Select distinct(CONVERT(char(10),SALARY_TAKE_OUT_DT, 120)) SALARY_TAKE_OUT_DT,STATUS 
                        From TB_D_R_RES_ACTURL 
                        Where MANAGER_DT > @DUTY_SDT and MANAGER_DT < DATEADD(DD,1,@DUTY_EDT )  and MEALSHIFT = 'D'");
    ...
    public DataTable checkSalaryClose()
    {
        ...
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("SELECT SALARY_LOCKED,Convert(varchar(10), a.SALARY_DT,111) SALARY_DT FROM TB_S_M_SALARY_MONTH_CTRL a");
            ...
            sb.Append(" where a.OPERATION_ID = 'K01' and a.SALARY_YM = @SALARY_YM order by SALARY_DT desc");
    ...
    public DataTable getData(int startRowIndex, int maximumRows, string sortExpression, string manager_ym_s, string manager_ym_e)
    {
        try
        {
            ...
            sb.Append(@" MANAGER_YM,CASE WHEN CONVERT(char(10),SALARY_TAKE_OUT_DT, 120) ='9999-12-31' THEN '' ELSE CONVERT(char(10),SALARY_TAKE_OUT_DT, 111) END SALARY_TAKE_OUT_DT,
			             STATUS,REMARK,CONVERT(char(10),GIVE_SALARY_DT, 111) GIVE_SALARY_DT
                         from (
			             select distinct(left(CONVERT(char,MANAGER_DT,112),6)) MANAGER_YM,SALARY_TAKE_OUT_DT,STATUS,REMARK,GIVE_SALARY_DT from TB_D_R_RES_ACTURL 
			             where left(CONVERT(char,MANAGER_DT,112),6)  between @manager_ym_s and @manager_ym_e  and MEALSHIFT = 'D' and EXPENSE_CD = '2'
                         group by left(CONVERT(char,MANAGER_DT,112),6),SALARY_TAKE_OUT_DT,STATUS,REMARK,GIVE_SALARY_DT
			             ) a
                         where 1=1");
    ...
    public int getCount(int startRowIndex, int maximumRows, string manager_ym_s, string manager_ym_e)
    {
        try
        {
            ...
            sb.Append(@"select distinct(left(CONVERT(char,MANAGER_DT,112),6)) MANAGER_YM,SALARY_TAKE_OUT_DT,STATUS,REMARK,GIVE_SALARY_DT from TB_D_R_RES_ACTURL 
			             where left(CONVERT(char,MANAGER_DT,112),6)  between @manager_ym_s and @manager_ym_e  and MEALSHIFT = 'D' and EXPENSE_CD = '2'
                         group by left(CONVERT(char,MANAGER_DT,112),6),SALARY_TAKE_OUT_DT,STATUS,REMARK,GIVE_SALARY_DT
			             ) b ");
    ...
    public DataTable getSalaryCode(string GIVE_SALARY_DT)
    {
        DBConnector dbConn = new DBConnector();
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("SELECT SALARY_YM,CONVERT(char(10),SALARY_DT, 120) SALARY_DT,SALARY_TYPE,CONVERT(char(10),SALARY_SDT, 120)SALARY_SDT,CONVERT(char(10),SALARY_EDT, 120)SALARY_EDT,CONVERT(char(10),DUTY_SDT, 111)DUTY_SDT,CONVERT(char(10),DUTY_EDT, 111)DUTY_EDT");
            sb.Append(" FROM TB_S_M_SALARY_CAL_H");
            sb.Append(" where SALARY_YM = @MANAGER_YM");
            if (GIVE_SALARY_DT != "")
            {
                sb.Append(" and SALARY_DT = @SALARY_DT");
                ht.Add("@SALARY_DT", GIVE_SALARY_DT);
            }

            sb.Append(" and SALARY_TYPE = 'A' order by SALARY_DT desc");
    ...
    public DataTable getSalaryCTL()
    {
        ...
        try
        {
            ...
            sb.Append(" where OPERATION_ID = 'K01' and SALARY_YM = @SALARY_YM");
            sb.Append(" and SALARY_DT = @SALARY_DT");
    ...
    public DataTable update_Month()
    {
        ...
        try
        {
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append("update TB_D_R_RES_ACTURL");
            ...
            sb.Append(" where MANAGER_DT > @DUTY_SDT and MANAGER_DT < DATEADD(DD,1,@DUTY_EDT )  and MEALSHIFT = 'D' and EXPENSE_CD = '2'");
    ...
    public DataTable update_MONTH_FIN()
    {
        ...
        try
        {
            ...
            ht.Add("@TAKE_OUT_BY", TAKE_OUT_BY);
            ht.Add("@SALARY_DT", SALARY_DT);
            ht.Add("@REMARK", DUTY_SDT + "~" + DUTY_EDT);
            ht.Add("@DUTY_SDT", DUTY_SDT);
            ht.Add("@DUTY_EDT", DUTY_EDT);
