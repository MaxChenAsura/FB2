...
/// CFB2IA3210BO 的摘要描述
/// </summary>
public class CFB2IA3210BO : BaseService
{
    public CFB2IA3210BO()
    {
        //
    ...
    public string delBILLS_COMPARE(ArrayList datas, CFB2IA3210DAO dao)
    {
        try
        {
            BeginTransaction();
            ...
            {
                dao.EMP_ID = item[0];
                dao.LICENSE_ID = item[1];

                dao.deleteBILLS_COMPARE();//保費比對異常資料檔          
            }
    ...
    public string getBoss(CFB2IA3210DAO dao)
    {
        try
        {
            string st = dao.getBoss();
            return st;
        }
        catch (Exception ex)
        {
    ...
    public string executeOK(CFB2IA3210DAO dao)
    {
        try
        {
            BeginTransaction();
            //保費追溯資料檔
            dao.insertTB_I_M_FEES_TRACEBACK();
            //其他加扣款檔
            dao.insertTB_S_M_SUBSIDY_DEDUCTIONS_1();
            //保費比對異常資料檔
            dao.updateBILLS_COMPARE("OK");
    ...
    public string checkToSalary(CFB2IA3210DAO dao)
    {
        try
        {
            string msg = "0";
            string st = "";
            DataTable dt = dao.checkToSalary();
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (dt.Rows[i]["SALARY_STATUS"].ToString() == "Y") {
                        msg = "其他加扣款已薪資處理，不可反確認!!!";
                    }
                }
            }
            else
            {
                msg = "目前薪資待處理月份已無可反確認資料!!";
            }
    ...
    public string executeNOTOK(CFB2IA3210DAO dao)
    {
        try
        {
            DataTable dt = dao.checkToSalary();
            DataTable dt1 = dao.selectTB_I_M_FEES_TRACEBACK();
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    dao.DATA_YM = dt.Rows[i]["DATA_YM"].ToString();
                    dao.TEMP_ID = dt.Rows[i]["EMP_ID"].ToString();
                    dao.SALARY_ID = dt.Rows[i]["SALARY_ID"].ToString();
                    dao.SEQ_NO = dt.Rows[i]["SEQ_NO"].ToString();
                    dao.delTB_S_M_SUBSIDY_DEDUCTIONS_1();
                }
            }
            //保費追溯資料檔
            if (dt1.Rows.Count > 0)
            {
                ...
                {
                    dao.SALARY_YM = dt1.Rows[i]["SALARY_YM"].ToString();
                    dao.TEMP_ID = dt1.Rows[i]["EMP_ID"].ToString();
                    dao.INS_TYPE = dt1.Rows[i]["INS_TYPE"].ToString();
                    dao.IDENTITY_KIND = dt1.Rows[i]["IDENTITY_KIND"].ToString();
                    dao.TLICENSE_ID = dt1.Rows[i]["LICENSE_ID"].ToString();
                    dao.TRACE_KIND = dt1.Rows[i]["TRACE_KIND"].ToString();
                    dao.delTB_I_M_FEES_TRACEBACK();
                }
            }
            
            //保費比對異常資料檔
            dao.updateBILLS_COMPARE("NOT");
