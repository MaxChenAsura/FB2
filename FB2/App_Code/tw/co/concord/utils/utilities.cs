...
public static class utilities
{
    ...
    public static void SendMail2(
        ...
        , string send
        , List<string> mailto
        , bool html = false, string file_name = "", Stream attch = null)
    {
    ...
    /// <param name="file_name">附件名稱</param>
    /// <param name="attch">附件</param>
    public static void SendMail(string subject, string content, string send, List<string> mailto
        , bool html = false
        , string file_name = ""
        , Stream attch = null)
    {
    ...
    /// <param name="content">內容</param>
    /// <param name="send">寄信者</param>
    /// <param name="RecipientID">收信者工號</param>
    /// <param name="RecipientEMAIL">收信者Email</param>
    /// <param name="html">是否為html</param>
    /// <param name="file_name">附件名稱</param>
    /// <param name="attch">附件</param>
        ...
        , Stream attch = null)
    {
        try
        {
            //string photoPath = ConfigurationManager.AppSettings["reportPath"];//圖片路徑  << 附圖相關程式3
            LinkedResource img1 = new LinkedResource(photoPath, "image/gif"); //<< 附圖相關程式4  我是用gif檔
            img1.ContentId = "Image1"; //<< 附圖相關程式5
            string mail_Address = "";
            string smtp_server = ConfigurationManager.AppSettings["smtpServer"];
            ...
            msgMail.From = new MailAddress(send);
            //收件者   mailcount            
            foreach (var item in RecipientEMAIL)
            {
                if (item != "" && item != null)
                {
                    mail_Address = item;
            ...
            {
                AlternateView alt = AlternateView.CreateAlternateViewFromString(pcontect, null, "text/html");
                alt.LinkedResources.Add(img1);     //附圖相關程式6
                msgMail.AlternateViews.Add(alt);
            }
            else{
                    msgMail.Body = pcontect;
    ...
    */
    ...
    //取得SP的錯誤訊息, SP名稱,14碼的時間
    public static DataTable getSPLOGDT(string proc_id, string PROC_DT)
    {
        try
        {
            DBConnector dbConn = new DBConnector();
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append(@" select PROC_STATUS,PROC_LOG 
                        from TB_SP_LOG
                        where 1=1 
                        and PROC_ID=@PROC_ID   ");
            if (PROC_DT != "")
            {
                sb.Append(" and PROC_DT=@PROC_DT ");
                ht.Add("@PROC_DT", PROC_DT);
            }
    ...
    //取得SP的錯誤訊息, SP名稱 
    public static DataTable getSPLOGDESC(string proc_id)
    {
        try
        {
            DBConnector dbConn = new DBConnector();
            StringBuilder sb = new StringBuilder();
            Hashtable ht = new Hashtable();
            sb.Append(@" select top 1 PROC_STATUS,PROC_LOG 
                        from TB_SP_LOG
                        where 1=1 
                        and PROC_ID=@PROC_ID   
