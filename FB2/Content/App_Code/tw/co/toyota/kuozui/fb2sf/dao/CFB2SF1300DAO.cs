...
public class CFB2SF1300DAO : BaseDAO
{
    ...
    public DataTable get_PDF_Data(string ACCT_ID,string Lno)
    {
        try
        {
            ...
            sb.Append(" select Convert(varchar(10),a.SALARY_DT ,111) as SALARY_DT,c.VENDOR_ID+'-'+c.CREDITOR as VENDOR_ID,b.SALARY_NAME,a.AMOUNT from TB_S_M_ALLOCATION_D a	");
            sb.Append(" left join VW_SALARYAND9999 b on b.SALARY_ID=a.PAY_KIND	");
            sb.Append(" left join TB_S_M_ARREARS_TARGET c on a.EMP_ID=c.EMP_ID and a.DOC_NO=c.DOC_NO and a.SEQ=c.SEQ ");
            sb.Append(" where 1=1 ");
            if (ACCT_ID != "")
            {
                sb.Append(" and ACCT_ID = @ACCT_ID");
