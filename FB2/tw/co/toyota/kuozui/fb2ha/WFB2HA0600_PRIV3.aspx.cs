...
public partial class WebContent_fb2ha_WFB2HA0600_PRIV3 : BasePage
{
    //Service 物件
    private CFB2HA0600_PRIV3BO service = new CFB2HA0600_PRIV3BO();
    ...
    {
        CFB2HA0600_PRIV3DAO fb2ha = new CFB2HA0600_PRIV3DAO();
    ...
    protected void WFB2HA0600Delete_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            for (int i = 0; i < this.gv_result.Rows.Count; i++)
            {
                ...
                {
                    fb2ha.EMP_ID = ((Label)gv_result.Rows[i].FindControl("lbl_EMP_ID")).Text;
                    string deletekey = string.Format("{0}|{1}", gv_result.DataKeys[i].Value.ToString(), fb2ha.EMP_ID);
                    deleteList.Add(deletekey);
                }
            }
            if (deleteList.Count() == 0)
            {
                ScriptManager.RegisterClientScriptBlock(WFB2HA0600Delete, this.GetType(), "error", "alert('刪除請選擇一筆資料')", true);
                return;
            }
            else
            {
    ...
    protected void WFB2HA0600Save_Click(object sender, EventArgs e)
    {
        ...
        {
            CFB2HA0600_PRIV3DAO fb2ha = new CFB2HA0600_PRIV3DAO();
            CFB2HA0600_PRIV3BO service = new CFB2HA0600_PRIV3BO();
            string msg = "";
            ...
            if (gv_result.EditIndex == -1)
            {
                ...
                fb2ha.HR_CHG_CD = ((TextBox)KeyinRow.FindControl("txt_HR_CHG_CD_Add")).Text.ToUpper();
                fb2ha.HR_CHG_DESC = ((TextBox)KeyinRow.FindControl("txt_HR_CHG_DESC_Add")).Text;
                fb2ha.EMP_ID = ((TextBox)KeyinRow.FindControl("txt_EMP_ID_Add")).Text;
                fb2ha.EMP_NAME = ((TextBox)KeyinRow.FindControl("txt_EMP_NAME_Add")).Text.Trim();
                fb2ha.IS_VALID = "Y";
            ...
            else
            {
                ...
                fb2ha.HR_CHG_DESC = ((TextBox)KeyinRow.FindControl("txt_HR_CHG_DESC_Add")).Text;
                fb2ha.EMP_ID = ((TextBox)KeyinRow.FindControl("txt_EMP_ID_Add")).Text;
                fb2ha.EMP_NAME = ((TextBox)KeyinRow.FindControl("txt_EMP_NAME_Add")).Text;
