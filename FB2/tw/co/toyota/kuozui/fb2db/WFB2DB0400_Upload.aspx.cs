...
public partial class WebContent_fb2db_WFB2DB0400_Upload : BasePage
{
    //Service 物件
    ...
    public void exportExcel()
    {
        try
        {
            if (Session["FileType_DB0400"] != null && Session["FileType_DB0400"].ToString() != "")
            {
                string FileType_DB0400 = Session["FileType_DB0400"].ToString();
                if (FileType_DB0400 == "excel")
                {
                    Session["FileType_DB0400"] = "";
                    ExcelHandle.excel_Down(Server.MapPath("~/ExcelTemplate/DownloadFile/FF2DB040_error_" + SessionHandle.Current.emp_id + ".xlsx"), "error.xlsx");
                }
            }
        }
        catch (Exception ex)
    ...
    protected void WFB2DB0400Upload_Click(object sender, EventArgs e)
    {
        string msg = "";
        try
        {
            if (FileUpload1.HasFile)
            {
                ...
                //先刪除原始的檔案
                string toPath = Server.MapPath("~/ExcelTemplate/DownloadFile/FF2DB040_error_" + SessionHandle.Current.emp_id + ".xlsx");
                File.Delete(toPath);
                if (workbook == null)
                {
                ...
                else
                {
                    ...
                    dwnframe.Attributes["src"] = "WFB2DB0400_Upload.aspx?FileType_DB0400=excel";
                    Session["FileType_DB0400"] = "excel";
    ...
    {
        if (File.Exists(Server.MapPath("~/ExcelTemplate/WFB2DB040_Upload.xlsx")))
        {
            try
            {
                ExcelHandle.excel_Down(Server.MapPath("~/ExcelTemplate/WFB2DB040_Upload.xlsx"), "WFB2DB040_Upload.xlsx");
            }
            catch (Exception ex)
            {
                logger.Error(ex.Message);
    ...
    {
        Session["DB0400_Is_Search"] = "Y";
        Response.Redirect("WFB2DB0400_Qry.aspx");
    }
