...
public partial class WebContent_fb2da_WFB2DA0100_Upload : BasePage
{
    //Service 物件
    ...
    public void exportExcel()
    {
        try
        {
            if (Session["FileType_DA0100"] != null && Session["FileType_DA0100"].ToString() != "")
            {
                string FileType_DA0100 = Session["FileType_DA0100"].ToString();
                if (FileType_DA0100 == "excel")
                {
                    Session["FileType_DA0100"] = "";
                    ExcelHandle.excel_Down(Server.MapPath("~/ExcelTemplate/DownloadFile/FF2DA010_error_" + SessionHandle.Current.emp_id + ".xlsx"), "error.xlsx");
                }
            }
        }
        catch (Exception ex)
    ...
    protected void WFB2DA0100Upload_Click(object sender, EventArgs e)
    {
        string msg = "";
        try
        {
            if (FileUpload1.HasFile)
            {
                ...
                //先刪除原始的檔案
                string toPath = Server.MapPath("~/ExcelTemplate/DownloadFile/FF2DA010_error_" + SessionHandle.Current.emp_id + ".xlsx");
                File.Delete(toPath);
                if (workbook == null)
                {
                ...
                else
                {
                    ...
                    dwnframe.Attributes["src"] = "WFB2DA0100_Upload.aspx?FileType_DA0100=excel";
                    Session["FileType_DA0100"] = "excel";
    ...
    {
        if (File.Exists(Server.MapPath("~/ExcelTemplate/WFB2DA010_Upload.xlsx")))
        {
            try
            {
                ExcelHandle.excel_Down(Server.MapPath("~/ExcelTemplate/WFB2DA010_Upload.xlsx"), "WFB2DA010_Upload.xlsx");
            }
            catch (Exception ex)
            {
                logger.Error(ex.Message);
