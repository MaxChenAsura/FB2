...
public partial class WebContent_WFB2IA3210_Qry : BasePage
{
    CFB2IA3210BO service = new CFB2IA3210BO();
    ...
    private void getQryField()
    {
        try
        {
            ...
            WFB2IA3210Search_Click(null, null);
    ...
    private void geType()
    {
        try
        {
            ...
            ddl_COMP_PORCESS_YN.Items.Add(new ListItem("已確認", "Y"));
            ddl_COMP_PORCESS_YN.Items.Add(new ListItem("未確認", "N"));
    ...
    private void getGridView(string SortExpression, int pageindex, Int32 pagesize)
    {
        try
        {
            ...
            gv_result.DataKeyNames = new string[] { "EMP_ID", "LICENSE_ID" }; //設定GridView Key
            gv_result.DataBind();
/*
    ...
    protected void gv_result_RowCreated(object sender, GridViewRowEventArgs e)
    {
        ...
        if(grid.ID == "gv_result")
        {
            ...
            if (e.Row.RowType == DataControlRowType.Header)
            {
                ...
                //TableCell tcMergeProduct2 = new T
                //tcMergeProduct.CoableCell();
                //tcMergeProduct.Text = "本月保費";lumnSpan = 3;
...
*/
    ...
    #region "Button Event"
    //查詢按鈕事件
    protected void WFB2IA3210Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            setQryField();
            string COMPANY_CD = txt_COMPANY_CD.Text;
            CFB2IA3210DAO fb2ia = new CFB2IA3210DAO();
            ...
            if (gv_result.Rows.Count == 0)
            {
                ...
                WFB2IA3210Delete.Visible = false;
                WFB2IA3210OK.Visible = false;
                WFB2IA3210NOTOK.Visible = false;
                gv_result.Visible = false;               
            }
            else
            {
                WFB2IA3210Delete.Visible = true;
                gv_result.Visible = true;    

                if (ddl_COMP_PORCESS_YN.SelectedValue == "Y")
                {
                    WFB2IA3210OK.Visible = false;
                    WFB2IA3210NOTOK.Visible = true;
                }
                else
                {
                    WFB2IA3210OK.Visible = true;
                    WFB2IA3210NOTOK.Visible = false;                    
                }                
...
*/
    ...
    //刪除
    protected void WFB2IA3210Delete_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < this.gv_result.Rows.Count; i++)
            {
                if (((CheckBox)gv_result.Rows[i].FindControl("cb_check")).Checked)
                {
                    datas.Add(new string[] { gv_result.DataKeys[i].Values["EMP_ID"].ToString()
                                             ,gv_result.DataKeys[i].Values["LICENSE_ID"].ToString()                                                                                   
                                        });
                }
            }
            CFB2IA3210DAO dao = new CFB2IA3210DAO();
            dao.BILLS_KIND = ddl_BILLS_KIND.SelectedValue;
            dao.FEES_YM = txt_FEES_YM.Text.Replace("/","");
            dao.COMPANY_CD = txt_COMPANY_CD.Text;

            string msg = service.delBILLS_COMPARE(datas, dao);
    ...
    //確認
    protected void WFB2IA3210OK_Click(object sender, EventArgs e)
    {
        //查詢條件
        CFB2IA3210DAO dao = new CFB2IA3210DAO();
        dao.BILLS_KIND = ddl_BILLS_KIND.SelectedValue;//帳單匯入種類
        dao.FEES_YM = txt_FEES_YM.Text.Replace("/", "");//計費年月
        dao.COMPANY_CD = txt_COMPANY_CD.Text;//聘用單位
        dao.EMP_ID = txt_EMP_ID.Text;
        dao.LICENSE_ID = txt_LICENSE_ID.Text;
        dao.COMP_PORCESS_YN = ddl_COMP_PORCESS_YN.SelectedValue;
        dao.APPROVE_BY = service.getBoss(dao);
        dao.CREATED_BY = SessionHandle.Current.emp_id;
        dao.UPDATED_BY = SessionHandle.Current.emp_id;
        dao.FUNC_ID = "FB2IA321";

        string msg = service.executeOK(dao);
    ...
    //反確認
    protected void WFB2IA3210NOTOK_Click(object sender, EventArgs e)
    {
        //查詢條件
        CFB2IA3210DAO dao = new CFB2IA3210DAO();
        dao.BILLS_KIND = ddl_BILLS_KIND.SelectedValue;//帳單匯入種類
        ...
        //檢核 1 其他加扣款已經薪資處理，不能反確認
        string msg = service.checkToSalary(dao);
        if (msg != "0")
        {
