...
public partial class WebContent_fb2sm_WFB2SM2200_Dtl : BasePage
{
    private enum UIMode
    {
        Init,
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            CFB2SM2200DAO fb2sm = new CFB2SM2200DAO();
            ...
            if (dataCount == 0)
            {
                Session["SM2200_Is_Search"] = "Y";
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "noData", "alert('查無明細資料!');$(location).attr('href','WFB2SM2200_Qry.aspx');", true);
                return;
            }
            else
            {
    ...
    {
        CFB2SM2200DAO dao = new CFB2SM2200DAO();
        ...
        {
            WFB2SM2200Confirm.Enabled = false;
            WFB2SM2200Reject.Enabled = false;
    ...
    protected void WFB2SM120Confirm_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            CFB2SM2200BO bo = new CFB2SM2200BO();
            string msg = bo.updateConfirmData(confirmList, hid_QDATAKEY.Value, txt_REMARK_DESC.Text);//核可更新明細
            if (msg != "0")
            {
                msg = msg.Replace("\r\n", "");
                msg = msg.Replace("'", "");
                ScriptManager.RegisterClientScriptBlock(WFB2SM2200Confirm, this.GetType(), "WFB2SM220Confirm_Error", "alert('" + msg + "'); $.unblockUI();", true);
            }
            else
            {
                Session["SM2200_Is_Search"] = "Y";
                ScriptManager.RegisterClientScriptBlock(WFB2SM2200Confirm, this.GetType(), "WFB2SM120Confirm_Clicksuccess", "alert($('#hidwfb2sm_Confirm_Clicksuccess').val());$(location).attr('href','WFB2SM2200_Qry.aspx');", true);
            }
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(WFB2SM2200Confirm, this.GetType(), "WFB2SM120Confirm_Clickerror", "alert('" + ex.Message + "');", true);
        }
    ...
    protected void WFB2SM120Reject_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            CFB2SM2200BO bo = new CFB2SM2200BO();
            string msg = bo.updateRemain(remainList, rejectList, hid_QDATAKEY.Value, txt_REMARK_DESC.Text);
            if (msg != "0")
            {
                msg = msg.Replace("\r\n", "");
                msg = msg.Replace("'", "");
                ScriptManager.RegisterClientScriptBlock(WFB2SM2200Reject, this.GetType(), "WFB2SM2200Reject_Error", "alert('" + msg + "'); $.unblockUI();", true);
            }
            else
            {
                Session["SM2200_Is_Search"] = "Y";
                ScriptManager.RegisterClientScriptBlock(WFB2SM2200Reject, this.GetType(), "WFB2SM120Reject_Clicksuccess", "alert($('#hidwfb2sm_Reject_Clicksuccess').val());$(location).attr('href','WFB2SM2200_Qry.aspx');", true);
            }
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(WFB2SM2200Reject, this.GetType(), "WFB2SM120Reject_Clickerror", "alert('" + ex.Message + "');", true);
        }
    ...
    {
        Session["SM2200_Is_Search"] = "Y";
        Response.Redirect("WFB2SM2200_Qry.aspx");
    }
    ...
    private void EditOrAddMode(UIMode uimode, int EditIndex)
    {
        switch (uimode)
        {
            case UIMode.Query:
            case UIMode.Cancel:
                WFB2SM2200Confirm.Visible = true;
                WFB2SM2200Reject.Visible = true;
