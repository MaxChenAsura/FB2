...
public partial class WebContent_fb2sm_WFB2SM1200_Qry : BasePage
{
    private enum UIMode
    {
        Init,
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                WFB2SM1200Search_Click(sender, e);
            }
        }
    ...
    private void getQryField()
    {
        try
        {
            txt_DATA_YEAR_search.Text = Session["SM1200_DATA_YEAR"].ToString();
            ViewState["PerPageRow"] = Session["SM1200_ddlPerPageRow"].ToString();
            hid_firstLoad.Value = "N";
            WFB2SM1200Search_Click(null, null);
            Session["SM1200_Is_Search"] = "N";
        }
        catch
        {
        }
    ...
    {
        Session["SM1200_DATA_YEAR"] = txt_DATA_YEAR_search.Text;
    }
    #endregion
    ...
    private void getGridView(string SortExpression, int pageindex, Int32 pagesize)
    {
        try
        {
            ...
            if (gv_result.Rows.Count == 0)
            {
                ...
                WFB2SM1200Detail.Visible = true;
            }
            HID_PageRow.Value = "";
            Session["SM1200_ddlPerPageRow"] = ViewState["PerPageRow"];
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(WFB2SM1200Search, this.GetType(), "error", "alert('" + ex.Message + "');", true);
        }
    }
    ...
    //查詢按鈕事件
    protected void WFB2SM1200Search_Click(object sender, EventArgs e)
    {
        try
        {
            setQryField();
    ...
    //查詢明細按鈕事件
    protected void WFB2SM1200Detail_Click(object sender, EventArgs e)
    {
        try
        {
            //檢查勾選項目
            ...
            {
                ScriptManager.RegisterClientScriptBlock(WFB2SM1200Detail, this.GetType(), "error", "alert($('#wfb2sm_Dtl_NotChoiceMessage')", true);
                return;
            }
            else
            {
                string aaa = detailList[0];
                Response.Redirect("WFB2SM1200_Dtl.aspx?1=1&qdatakey=" + detailList[0]);
            }
        }
        catch (Exception ex)
        {
            //logger.Error(ex.Message);
            ScriptManager.RegisterClientScriptBlock(WFB2SM1200Detail, this.GetType(), "error", "alert('" + ex.Message + "');", true);
        }
    }
    ...
    private void EditOrAddMode(UIMode uimode, int EditIndex)
    {
        switch (uimode)
        {
            ...
            case UIMode.Init:
                WFB2SM1200Search.Enabled = true;
                btn_clear.Enabled = true;
                WFB2SM1200Detail.Visible = false;
                gv_result.EditIndex = -1;
                this.gv_result.Visible = true;
