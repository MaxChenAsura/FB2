...
public partial class WebContent_fb2sl_WFB2SL4100_Qry : BasePage
{
    private enum UIMode
    {
        Init,
    ...
    //Service 物件
    private CFB2SL4100BO service = new CFB2SL4100BO();
    ...
    protected void hid_getEmpName_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            CFB2SL4100BO service = new CFB2SL4100BO();
    ...
    private void getQryField()
    {
        try
        {
            txt_DATA_YM_search.Text = Session["SL4100_DATA_YM"].ToString();
            lb_SALARY_DT.Text = Session["SL4100_SALARY_DT"].ToString();
            txt_DEPT_NO.Text = Session["SL4100_DEPT_NO"].ToString();
            txt_DEPT_NAME.Text = Session["SL4100_DEPT_NAME"].ToString();
            ddl_WS_CD_search.SelectedValue = Session["SL4100_WS_CD"].ToString();
            //rbl_BORROW_TYPE.SelectedValue = Session["SL4100_BORROW_TYPE"].ToString();
            txt_EMP_ID.Text = Session["SL4100_EMP_ID"].ToString();
            txt_EMP_DESC.Text = Session["SL4100_EMP_DESC"].ToString();
            txt_LICENSE_ID_search.Text = Session["SL4100_LICENSE_ID"].ToString();
            ddl_EMP_STATUS.SelectedValue = Session["SL4100_EMP_STATUS"].ToString();
            ViewState["PerPageRow"] = Session["SL4100_ddlPerPageRow"].ToString();

            WFB2SL4100Generate_Click(null, null);
            Session["SL4100_Is_Search"] = "N";
        }
        catch
        {
        }
    ...
    {
        Session["SL4100_DATA_YM"] = txt_DATA_YM_search.Text;
        Session["SL4100_SALARY_DT"] = lb_SALARY_DT.Text;
        Session["SL4100_DEPT_NO"] = txt_DEPT_NO.Text;
        Session["SL4100_DEPT_NAME"] = txt_DEPT_NAME.Text;
        Session["SL4100_WS_CD"] = ddl_WS_CD_search.SelectedValue;
        //Session["SL4100_BORROW_TYPE"] = rbl_BORROW_TYPE.SelectedValue;
        Session["SL4100_EMP_ID"] = txt_EMP_ID.Text;
        Session["SL4100_EMP_DESC"] = txt_EMP_DESC.Text;
        Session["SL4100_LICENSE_ID"] = txt_LICENSE_ID_search.Text;
        Session["SL4100_EMP_STATUS"] = ddl_EMP_STATUS.SelectedValue;
    }
    #endregion
    ...
    private void getGridView(string SortExpression, int pageindex, Int32 pagesize)
    {
        try
        {
            ...
            HID_PageRow.Value = "";
            Session["SL4100_ddlPerPageRow"] = ViewState["PerPageRow"];
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(WFB2SL4100Generate, this.GetType(), "error", "alert('" + ex.Message + "');", true);
        }
    }
    ...
    protected void gv_result_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "ToDetail")
        {
            ...
            Session["SL4100_DTL_QKEY"] = gv_result.DataKeys[index].Value.ToString();
            //Response.Redirect("WFB2SL4100_Dtl.aspx?1=1&qdatakey=" + qdatakey);
            Response.Redirect("WFB2SL4100_Dtl.aspx");
        }
    }
    #endregion
    ...
    #region "Button Event"
    //查詢按鈕事件
    protected void WFB2SL4100Generate_Click(object sender, EventArgs e)
    {
        try
        {
            bool is_qry = false;
            List<string> Emps = utilities.getAcesEMP_LIST();
            //if (rbl_BORROW_TYPE.SelectedValue == "1" && Emps.Contains(txt_EMP_ID.Text.Trim()))
    ...
    private void EditOrAddMode(UIMode uimode, int EditIndex)
    {
        switch (uimode)
        {
            ...
            case UIMode.Init:
                ...
                WFB2SL4100Generate.Enabled = true;
                btn_clear.Enabled = true;
                this.gv_result.ShowFooter = false;
                gv_result.EditIndex = -1;
                this.gv_result.Visible = false;
