...
public partial class WebContent_fb2di_WFB2DI0200_Qry : BasePage
{
    //Service 物件
    private CFB2DI0200BO service = new CFB2DI0200BO();
    ...
    private void getGridView(string SortExpression, int pageindex, Int32 pagesize)
    {
        try
        {
            ...
            if (ViewState["SortExpression"] == null)
                getSortDirection("WORK_CD,START_DT");

            //GridView基本設定
            gv_result.PageIndex = 0;
            gv_result.PageSize = pagesize;
            gv_result.DataSourceID = "ods1";
            gv_result.DataKeyNames = new string[] { "DEPT_NO", "WORK_CD", "START_DT" }; //設定GridView Key
            gv_result.DataBind();
    ...
    protected void WFB2DI0200Search_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //GridView有分頁此段必加 begin
            if (ViewState["PerPageRow"] != null && ViewState["PerPageRow"].ToString() != "")
                getGridView("WORK_CD,START_DT", 0, Convert.ToInt32(ViewState["PerPageRow"]));
            else
                getGridView("WORK_CD,START_DT", 0, 10);
            //end
            ...
            {
                WFB2DI0200Add.Visible = true;
                WFB2DI0200Delete.Visible = true;
                WFB2DI0200Edit.Visible = true;
            }
            else
            {
                WFB2DI0200Delete.Visible = false;
                WFB2DI0200Edit.Visible = false;
    ...
    protected void WFB2DI0200Add_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ViewState["Queryble"] = true;
            //隱藏查詢清除按鈕
            WFB2DI0200Search.Visible = false;
            ...
            WFB2DI0200Save.Visible = true;
            btn_Cancel.Visible = true;

            WFB2DI0200Add.Visible = false;
            WFB2DI0200Delete.Visible = false;
            WFB2DI0200Edit.Visible = false;
            gv_result.EditIndex = -1;
    ...
    protected void WFB2DI0200Delete_Click(object sender, EventArgs e)
    {
        try
        {
            //檢查勾選項目 
            List<Tuple<string, string, string, string>> dept_no = new List<Tuple<string, string, string, string>>();
            for (int i = 0; i < this.gv_result.Rows.Count; i++)
            {
                if (((CheckBox)gv_result.Rows[i].FindControl("cb_check")).Checked)
                {
                    dept_no.Add(new Tuple<string, string, string, string>(
                        gv_result.DataKeys[i].Values["DEPT_NO"].ToString().Split('-')[0],
                        gv_result.DataKeys[i].Values["WORK_CD"].ToString().Split('-')[0],
                        Convert.ToDateTime(gv_result.DataKeys[i].Values["START_DT"]).ToString("yyyy/MM/dd"),
                        ((Label)gv_result.Rows[i].FindControl("lb_END_DT")).Text));
                }
            }

            string msg = service.deleteOVERTIME_SPECIAL_HOUR(dept_no);
            if (msg != "0")
            {
    ...
    protected void WFB2DI0200Edit_Click(object sender, EventArgs e)
    {
        try
        {
            gv_result.PagerSettings.Visible = false;
    ...
    protected void WFB2DI0200Save_Click(object sender, EventArgs e)
    {
        try
        {
            //無筆數新增
            if (gv_result.Rows.Count == 0)
            {
                ...
                //wfb2di.DEPT_NO = txt_NEW_DEPT_NO.Text;
                List<string> dept_no_list = getDEPT_NO(txt_NEW_DEPT_NO.Text);
                if (dept_no_list.Count == 0)
                    return;

                wfb2di.WORK_CD = ddl_NEW_WORK_CD.SelectedValue;
                wfb2di.START_DT = txt_NEW_START_DT.Text;
                if (txt_NEW_END_DT.Text == "")
                {
                    txt_NEW_END_DT.Text = "9999/12/31";
                }
                wfb2di.END_DT = txt_NEW_END_DT.Text;
                wfb2di.REMARK = txt_NEW_REMARK.Text;
                wfb2di.CREATED_BY = SessionHandle.Current.emp_id;
                wfb2di.UPDATED_BY = SessionHandle.Current.emp_id;
                wfb2di.FUNC_ID = "FB2DI020";

                string msg = service.addOVERTIME_SPECIAL_HOUR(wfb2di, dept_no_list);
                if (msg != "0")
                {
                    gv_result.PagerSettings.Visible = false;
            ...
            else
            {
                ...
                {
                    //更新
                    string start_dt = Convert.ToDateTime(gv_result.DataKeys[gv_result.EditIndex].Values["START_DT"]).ToString("yyyy/MM/dd");
                    ...
                    if (txt_END_DT.Text != "")
                    {
                        ...
                        DateTime edt = Convert.ToDateTime(txt_END_DT.Text);
                        if (sdt > edt)
                        {
                            gv_result.PagerSettings.Visible = false;
                            ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "error", "alert('生效日期不能大於結束日期');", true);
                            return;
                        }
                    }
                    else
                    ...
                    CFB2DI0200DAO wfb2di = new CFB2DI0200DAO();
                    wfb2di.DEPT_NO = gv_result.DataKeys[gv_result.EditIndex].Values["DEPT_NO"].ToString().Split('-')[0];
                    wfb2di.WORK_CD = gv_result.DataKeys[gv_result.EditIndex].Values["WORK_CD"].ToString().Split('-')[0];
                    ...
                    string msg = service.updateOVERTIME_SPECIAL_HOUR(wfb2di);
                    if (msg != "0")
                    {
                        gv_result.PagerSettings.Visible = false;
            ...
            gv_result.DataSourceID = "ods1";
            gv_result.DataKeyNames = new string[] { "DEPT_NO", "WORK_CD", "START_DT" };
            gv_result.EditIndex = -1;
            gv_result.ShowFooter = false;

            //顯示查詢清除按鈕
            WFB2DI0200Search.Visible = true;
            btn_clear.Visible = true;

            WFB2DI0200Save.Visible = false;
    ...
    private List<string> getDEPT_NO(string dept_no)
    {
        List<string> result = new List<string>();

        try
        {
            for (int i = 0; i < dept_no.Split(',').Length - 1; i++)
            {
                result.Add(dept_no.Split(',')[i].Split('-')[0]);
            }
