...
public partial class WebContent_fb2dl_WFB2DL0200_Qry : BasePage
{
    private enum UIMode
    {
        Init,
    ...
    private void getQryField()
    {
        try
        {
            txt_LEAVE_PLAN_YEAR_search.Text = Session["DL0200_LEAVE_PLAN_YEAR"].ToString();
            txt_DEPT_NO.Text = Session["DL0200_DEPT_NO"].ToString();
            txt_DEPT_NAME.Text = Session["DL0200_DEPT_NAME"].ToString();
            txt_EMP_ID_search.Text = Session["DL0200_EMP_ID"].ToString();
            txt_qry_EMP_NAME.Text = Session["DL0200__qry_EMP_NAME"].ToString();
            txt_IFLOW_NO.Text = Session["DL0200_IFLOW_NO"].ToString();
            hid_cb_leave_Hour_notEnough.Value = Session["DL0200_cb_leave_Hour_notEnough"].ToString();
            if (hid_cb_leave_Hour_notEnough.Value == "Y")
                cb_leaveHour_notEnough.Checked = true;
            else
                cb_leaveHour_notEnough.Checked = false;

            ViewState["PerPageRow"] = Session["DL0200_ddlPerPageRow"].ToString();

            WFB2DL0200Search_Click(null, null);
            Session["DL0200_Is_Search"] = "N";
        }
        catch
        {
        }
    ...
    {
        Session["DL0200_LEAVE_PLAN_YEAR"] = txt_LEAVE_PLAN_YEAR_search.Text;
        Session["DL0200_DEPT_NO"] = txt_DEPT_NO.Text;
        Session["DL0200_DEPT_NAME"] = txt_DEPT_NAME.Text;
        Session["DL0200_EMP_ID"] = txt_EMP_ID_search.Text;
        Session["DL0200__qry_EMP_NAME"] = txt_qry_EMP_NAME.Text;
        Session["DL0200_IFLOW_NO"] = txt_IFLOW_NO.Text;
        Session["DL0200_cb_leave_Hour_notEnough"] = hid_cb_leave_Hour_notEnough.Value;
    }
    #endregion
    ...
    private void getGridView(string SortExpression, int pageindex, Int32 pagesize)
    {
        try
        {
            ...
            if (ViewState["SortExpression"] == null)
                getSortDirection("LEAVE_PLAN_YEAR, ORI_DEPT_NO");    //排序方式(BasePage.cs)
            ...
            HID_PageRow.Value = "";
            Session["DL0200_ddlPerPageRow"] = ViewState["PerPageRow"];
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterClientScriptBlock(WFB2DL0200Search, this.GetType(), "error", "alert('" + ex.Message + "');", true);
        }
    }
    ...
    //查詢按鈕事件
    protected void WFB2DL0200Search_Click(object sender, EventArgs e)
    {
        try
        {
            setQryField();
    ...
    protected void WFB2DL0200Add_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("WFB2DL0200_Dtl.aspx?state=add&qdatakey=0");
        }
        catch (Exception ex)
        {
            //logger.Error(ex.Message);
            ScriptManager.RegisterClientScriptBlock(WFB2DL0200Add, this.GetType(), "error", "alert('" + ex.Message + "');", true);
        }
    }
    //刪除按鈕事件
    protected void WFB2DL0200Delete_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            int dataCount = dao.getCount(gv_result.PageSize * gv_result.PageIndex, ((gv_result.PageIndex + 1) * gv_result.PageSize), txt_LEAVE_PLAN_YEAR_search.Text
                                        , txt_DEPT_NO.Text, txt_EMP_ID_search.Text, txt_IFLOW_NO.Text, hid_cb_leave_Hour_notEnough.Value);
            if (dataCount == 0)
            {
                EditOrAddMode(UIMode.Init, -1);
            }
    ...
    protected void WFB2DL0200Edit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.Redirect("WFB2DL0200_Dtl.aspx?state=mod&qdatakey=" + qdatakeyList[0]);
        }
        catch (Exception ex)
        {
            //logger.Error(ex.Message);
    ...
    //查詢明細按鈕事件
    protected void WFB2DL0200Detail_Click(object sender, EventArgs e)
    {
        try
        {
            //檢查勾選項目
            ...
            Response.Redirect("WFB2DL0200_Dtl.aspx?state=detail&qdatakey=" + qdatakeyList[0]);
        }
        catch (Exception ex)
        {
            //logger.Error(ex.Message);
            ScriptManager.RegisterClientScriptBlock(WFB2DL0200Detail, this.GetType(), "error", "alert('" + ex.Message + "');", true);
        }
    }
