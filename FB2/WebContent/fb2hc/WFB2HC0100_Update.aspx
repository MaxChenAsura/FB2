<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="~/tw/co/toyota/kuozui/fb2hc/WFB2HC0100_Update.aspx.cs" Inherits="WebContent_WFB2HC0100_Update" Culture="auto" UICulture="auto" %>
                ...
                //if ($("#init_add").is(":visible")) {
                    if (check_EMP_ID()) {
                        ...
                        if ($("#hid_SysCodeAtt").val() == "Y") {
                            //OpenSearch('HrChangeCode_Search.aspx', 'txt_HR_CHG_CD', 'txt_HR_CHG_CD_DESC', 'HR_CHG_CD=' + $("#hid_EMP_CD").val() + '&IS_VALID=Y');
                            //__doPostBack('txt_HR_CHG_CD_change', '');
                            //HrChangeCode_Search.aspx 未處理員工區分
                            OpenSearch('HrChangeCode_Search.aspx', 'txt_HR_CHG_CD', 'txt_HR_CHG_CD_DESC', 'IS_VALID=Y');
                            __doPostBack('txt_HR_CHG_CD_change', '');
                        }
                        //若 資料權限之「小分類」為N(管理部擔當)，
                        //傳入(無)(畫面.人事異動代碼)(無, 無, E.員工區分, 'Y', 登入者帳號)，選取其中之一取得人事異動代碼及說明，顯示於畫面上；
                        if ($("#hid_SysCodeAtt").val() == "N") {
                            //OpenSearch('HrChangeCode_Search.aspx', 'txt_HR_CHG_CD', 'txt_HR_CHG_CD_DESC', 'HR_CHG_CD=' + $("#hid_EMP_CD").val() + '&IS_VALID=Y&EMP_ID=' + $("#hid_LOGIN_ID").val());
                            //__doPostBack('txt_HR_CHG_CD_change', '');
                            //HrChangeCode_Search.aspx 未處理員工區分
                            OpenSearch('HrChangeCode_Search.aspx', 'txt_HR_CHG_CD', 'txt_HR_CHG_CD_DESC', 'IS_VALID=Y&EMP_ID=' + $("#hid_LOGIN_ID").val());
                            __doPostBack('txt_HR_CHG_CD_change', '');
                        }
                        //若 資料權限之「小分類」為W(各單位擔當)，
                        //傳入(無)(畫面.人事異動代碼)('D', 無, E.員工區分, 'Y', 無)，選取其中之一取得人事異動代碼及說明，顯示於畫面上；
                        if ($("#hid_SysCodeAtt").val() == "W") {
                            //OpenSearch('HrChangeCode_Search.aspx', 'txt_HR_CHG_CD', 'txt_HR_CHG_CD_DESC', 'UPD_RIGHT_CD=D&HR_CHG_CD=' + $("#hid_EMP_CD").val() + '&IS_VALID=Y');
                            //__doPostBack('txt_HR_CHG_CD_change', '');
                            //HrChangeCode_Search.aspx 未處理員工區分
                            OpenSearch('HrChangeCode_Search.aspx', 'txt_HR_CHG_CD', 'txt_HR_CHG_CD_DESC', 'UPD_RIGHT_CD=D&IS_VALID=Y');
                            __doPostBack('txt_HR_CHG_CD_change', '');
                        }
                    }
                //}
                            ...
                            <tbody>   
                                ...
                                <tr>
									<td align="left" class="Body_label" colspan="8" >										
										...
										<legend class="Body_label"><font color="darkblue" ><asp:Label ID="lb_TRANSFER" runat="server" Text="<%$Resources:Resource,wfb2hc_lb_TRANSFER%>"></asp:Label></font></legend>
...
<%--									            <asp:Button ID="WFB2HC0100Detail_Delete" runat="server" Text="<%$Resources:Resource,btn_delete%>" OnClick="WFB2HC0100Detail_Delete_Click" />
                                            ...
                                            <asp:HiddenField ID="hid_PLAN_DESPATCH_DT" ClientIDMode="Static" runat="server" />  <%--預計派遣日--%>
                                            <asp:HiddenField ID="hid_PLAN_DESPATCH_NEXT_DT" ClientIDMode="Static" runat="server" />  <%--預計派遣日次日--%>
                                            <asp:HiddenField ID="hid_EMP_CD" ClientIDMode="Static" runat="server" />            <%--員工區分--%>
                                            <asp:HiddenField ID="hid_WS_CD" ClientIDMode="Static" runat="server" />             <%--職種--%>
                                            <asp:HiddenField ID="hid_LEVEL_CD" ClientIDMode="Static" runat="server" />          <%--資格--%>
                                            ...
                                            <asp:HiddenField ID="hid_IS_TEMP" ClientIDMode="Static" runat="server" />           <%--是否暫時狀態--%>
                                            <asp:HiddenField ID="hid_LEVEL_CD_AFTER" ClientIDMode="Static" runat="server" />    <%--明細06-資格之異動後代碼--%>
                                            <asp:HiddenField ID="hid_IS_PROFESSION_PJOB" ClientIDMode="Static" runat="server" />    <%--是否專業職務--%>                                                                                    
